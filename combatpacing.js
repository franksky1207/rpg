(function(){
 ...
 window.animateFight=async function(r,startPlayerHp,playerMax,enemyMax,roundText=""){
  let ehp=enemyMax,php=startPlayerHp,eventIndex=0;
  const events=Array.isArray(r?.events)?r.events:[];
  const actionDelay=r?.e?.kind==="boss"?MAIN_BOSS_DELAY:MAIN_NORMAL_DELAY;
  function consumePlayerAttackDrain(){
   ...
   if(drain?.type!=="drain")return 0;
   eventIndex++;
   return Math.max(0,Math.floor(Number(drain.healed)||0));
  }
  async function applyDrainAfterAttack(){
   const healed=consumePlayerAttackDrain();
   if(healed<=0)return;
   php=Math.min(playerMax,php+healed);
   setCombatHp(ehp,enemyMax,php,playerMax,`汲取恢復 ${healed} HP`);
