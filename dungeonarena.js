(function(){
 ...
 window.ARENA_POSITION_API_VERSION=Math.max(0,Math.floor(Number(ARENA_VERSION_PROFILE.positionApiVersion)||0));
 ...
 window.startArenaDungeon=function(positionId){if((Number(state.level)||1)<15)return;const rank=currentArenaRank(),position=positionById(positionId,rank);if(!position)return;if(dailyStatus().remaining<=0){view="dungeon-arena";resetArenaState();render();return;}arenaState=createArenaState({phase:"ready",rank,position,enemy:buildArenaEnemy(position.id,0,null,null,{rank})});view="dungeon-arena";render();};

 function collectStageBasePoints(stageIndex){const basePoints=Math.max(0,Math.floor(Number(arenaState.position?.stagePoints?.[stageIndex])||0));arenaState.roundBasePoints+=basePoints;return basePoints;}
 ...
 function readyHtml(){
  const d=arenaState.position,ds=dailyStatus(),base=Math.max(0,Math.floor(Number(d?.totalPoints)||0)),actual=adjustedFullClearPoints(d),rewardText=actual!==base?`${actual} VIP 積分（基礎 ${base}）`:`${base} VIP 積分`;
