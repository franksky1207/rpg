(function(){
 ...
 function newContinuousSummary(){return {runs:0,wins:0,totalExp:0,totalGold:0,convertedGold:0,soldGold:0,gearCount:0,keptCount:0,soldCount:0,stopReason:null};}
 ...
 function setHpUi(enemyHp,enemyMax,playerHp,playerMax,message){const eb=document.getElementById("combatEnemyBar"),eh=document.getElementById("combatEnemyHp"),pb=document.getElementById("combatPlayerBar"),ph=document.getElementById("combatPlayerHp"),msg=document.getElementById("combatMessage");if(eb)eb.style.width=`${Math.max(0,Math.min(100,enemyHp/enemyMax*100))}%`;if(eh)eh.textContent=`${Math.max(0,enemyHp)} / ${enemyMax}`;if(pb)pb.style.width=`${Math.max(0,Math.min(100,playerHp/playerMax*100))}%`;if(ph)ph.textContent=`${Math.max(0,playerHp)} / ${playerMax}`;if(msg)msg.textContent=message||"";}
 function pulse(target,text){const card=document.getElementById(target==="enemy"?"combatEnemyCard":"combatPlayerCard"),dmg=document.getElementById(target==="enemy"?"combatEnemyDamage":"combatPlayerDamage");if(card&&text!=="閃避"&&text!=="吸收"){card.classList.remove("hit");void card.offsetWidth;card.classList.add("hit");setTimeout(()=>card.classList.remove("hit"),260);}if(dmg){dmg.textContent=text;dmg.classList.remove("show");void dmg.offsetWidth;dmg.classList.add("show");}}
 async function animateBounty(result,startHp,playerMax){let ehp=result.e.hp,php=startHp;const delay=result.logs.length>80?25:result.logs.length>40?45:90;setHpUi(ehp,result.e.hp,php,playerMax,"開始戰鬥");await sleep(150);for(const line of result.logs){let m=line.match(/^你攻擊.+，(?:暴擊)?造成 (\d+) 點傷害。$/);if(m){const d=Number(m[1]);ehp=Math.max(0,ehp-d);pulse("enemy",line.includes("暴擊")?`暴擊 ${d}`:`-${d}`);setHpUi(ehp,result.e.hp,php,playerMax,line);await sleep(delay);continue;}if(line.includes("閃避了你的攻擊")){pulse("enemy","閃避");setHpUi(ehp,result.e.hp,php,playerMax,line);await sleep(delay);continue;}m=line.match(/^.+攻擊你，(?:暴擊)?造成 (\d+) 點傷害。$/);if(m){const d=Number(m[1]);php=Math.max(0,php-d);pulse("player",line.includes("暴擊")?`暴擊 ${d}`:`-${d}`);setHpUi(ehp,result.e.hp,php,playerMax,line);await sleep(delay);continue;}if(line.includes("你閃避了攻擊")){pulse("player","閃避");setHpUi(ehp,result.e.hp,php,playerMax,line);await sleep(delay);continue;}if(line.includes("吸收印記化解了傷害")){pulse("player","吸收");setHpUi(ehp,result.e.hp,php,playerMax,line);await sleep(delay);continue;}setHpUi(ehp,result.e.hp,php,playerMax,line);await sleep(delay);}}
 ...
 async function runBountyFight(){
  ...
  if(result.win){const tier=bountyState.tier,rewardLevel=clampGameLevel(bountyState.enemy?.level??state.level),mapIdx=bountyMapIndex(rewardLevel);rewardExp=bountyExpReward(tier,rewardLevel);rewardGold=bountyGoldReward(tier,rewardLevel);expResult=applyBountyExp(rewardExp);state.gold+=rewardGold;for(let i=0;i<tier.gearCount;i++){const item=bountyItem(bountyState.enemy,mapIdx),handled=addItem(item),sold=Math.max(0,Number(handled?.sold)||0);soldGold+=sold;rewardItems.push({item,sold,kept:handled?.kept===true});}}
  const combatEndHp=result.combatEndHp;healAfterRound();
  const roundResult={win:result.win,rewardExp,expResult,rewardGold,soldGold,rewardItems,turns:result.turns,combatEndHp};bountyState.result=roundResult;updateSummary(roundResult);battleBusy=false;
 ...
 window.getBountyTestSnapshot=function(){return bountyState.enemy?{phase:bountyState.phase,tier:{...bountyState.tier},enemy:{...bountyState.enemy},continuous:bountyState.continuous,stopRequested:bountyState.stopRequested,summary:{...bountyState.summary},daily:dailyStatus()}:null;};
})();