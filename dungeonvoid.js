(function(){
 ...
 function baseStats(floor){
  ...
  return {
   ...
   crit:VOID_MIRAGE_BASE_CRIT,dodge:VOID_MIRAGE_BASE_DODGE
  };
 }
 ...
 function bossNameForFloor(floor){
  ...
  return VOID_MIRAGE_BOSS_NAMES[Math.floor((withinCycle-1)/10)];
 }
 ...
 function rollTraits(floor){
  const pool=traitIds(),out=[],count=isBossFloor(floor)?2:1;
 ...
 function buildEnemy(floor,options={}){
  ...
  return applyTraits({
   name,floor:f,kind:"dungeon-void-mirage",isBossFloor:boss,...base,
   baseCrit:VOID_MIRAGE_BASE_CRIT,baseDodge:VOID_MIRAGE_BASE_DODGE,
 ...
 function runSnapshot(){
  ...
  return {
   active:!!voidMirageRun.active,phase:voidMirageRun.phase,startFloor:voidMirageRun.startFloor,
   currentFloor:voidMirageRun.currentFloor,lastClearedFloor:voidMirageRun.lastClearedFloor,
   cleared:voidMirageRun.cleared,points:voidMirageRun.points,totalTurns:voidMirageRun.totalTurns,
   averageTurns:voidMirageRun.cleared?round1(voidMirageRun.totalTurns/voidMirageRun.cleared):0,
   exitRequested:!!voidMirageRun.exitRequested,endedReason:voidMirageRun.endedReason||"",
 ...
 window.getVoidMirageConfig=function(){return {
  name:VOID_MIRAGE_NAME,unlockLevel:VOID_MIRAGE_UNLOCK_LEVEL,baseCrit:VOID_MIRAGE_BASE_CRIT,baseDodge:VOID_MIRAGE_BASE_DODGE,
  hpMultiplier:VOID_MIRAGE_HP_MULTIPLIER,atkMultiplier:VOID_MIRAGE_ATK_MULTIPLIER,defMultiplier:VOID_MIRAGE_DEF_MULTIPLIER,
  regularNames:VOID_MIRAGE_REGULAR_NAMES.slice(),bossNames:VOID_MIRAGE_BOSS_NAMES.slice()
 };};
 ...
 window.beginVoidMirageRun=function(){
  ...
  voidMirageRun={
   active:true,phase:"ready",startFloor,currentFloor:startFloor,lastClearedFloor:startFloor-1,
   cleared:0,points:0,totalTurns:0,exitRequested:false,endedReason:"",failedFloor:0,
   lastEnemy:null,lastResult:null,previousRegularName:lastRegularName||""
  };
  fullHeal();
  if(typeof save==="function")save(false);
 ...
 window.fightNextVoidMirageFloor=function(){
  ...
  voidMirageRun.cleared+=clear.advanced?1:0;voidMirageRun.points+=clear.gained;
  voidMirageRun.lastClearedFloor=floor;voidMirageRun.currentFloor=clear.nextFloor;voidMirageRun.phase="between";
  fullHeal();
  if(typeof save==="function")save(false);
