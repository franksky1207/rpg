(function(){
 ...
 function vipDetailsHtml(){
  const lv=vipLevel(),points=vipPoints(),hpAtk=lv,def=round1(lv*.5),rate=round1(lv*.5),nextPerk=VIP_PERKS.find(x=>x.level>lv)?.level||null;
  ...
  return `<div class="vip-detail-head"><div class="vip-detail-level">${status}</div><div class="vip-bonus-grid"><div><span>HP</span><b>+${hpAtk}%</b></div><div><span>ATK</span><b>+${hpAtk}%</b></div><div><span>DEF</span><b>+${def}%</b></div><div><span>暴擊</span><b>+${rate}%</b></div><div><span>閃避</span><b>+${rate}%</b></div></div><div class="vip-detail-note">每提升 1 級 VIP：HP／ATK +1%，DEF +0.5%，暴擊／閃避 +0.5%。已解鎖的 VIP 等級不會因消耗 VIP 積分而下降。</div></div><div class="vip-perk-list">${VIP_PERKS.map(p=>{const cls=p.level<=lv?"unlocked":p.level===nextPerk?"next":"locked";return `<div class="vip-perk-row ${cls}"><div><div class="vip-perk-level">VIP${p.level}</div><div class="vip-perk-threshold">${vipThreshold(p.level).toLocaleString()} 積分</div></div><div>${p.text}</div></div>`;}).join("")}</div>`;
 }
