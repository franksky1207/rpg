(function(){
 ...
 function run(){
  ...
  const report={passed:errors.length===0,version:VERSION,checkedAt:Date.now(),dataIntegrityPassed:data?.passed===true,galaxyStories:Number(data?.galaxyStories)||0,universeRegistry:universeIds.size,universeStoriesLoaded:Number(data?.universeStoriesLoaded)||0,targetStories:201,errors,warnings};window.STORY_RUNTIME_INTEGRITY_REPORT=report;if(!report.passed)console.error("[文明戰線] 劇情執行期完整性檢查失敗",report);else console.info("[文明戰線] 劇情執行期完整性檢查通過",report);return report;
 }
 window.STORY_RUNTIME_INTEGRITY_VERSION=VERSION;window.runCivilizationStoryRuntimeIntegrity=run;run();
})();