(function(){
 ...
 function rankOptions(){
  ...
  const current=typeof getArenaCurrentRank==="function"?getArenaCurrentRank():1;
  let html="";
  for(let rank=1;rank<=max;rank++){
   const name=typeof getArenaRankName==="function"?getArenaRankName(rank):`${WORLD_REGIONS?.[rank-1]?.name||`第${rank}區`}級`;
   html+=`<option value="${rank}" ${rank===current?"selected":""}>Rank ${rank}｜${name}</option>`;
  }
  return html;
 }
 ...
 function resultHtml(s,assessment=false){
  ...
  const rankName=typeof getArenaRankName==="function"?getArenaRankName(s.rank):`Rank ${s.rank}`;
  ...
  return `<div class="notice"><div class="gm-test-summary"><div class="gm-test-summary-title">${rankName}・${difficultyLabel(s.difficultyId)}・${s.runs} 次完整三連戰</div><div class="gm-test-summary-line">${vipText}</div><div class="gm-test-summary-line">${specText}</div></div><div class="muted gm-test-context">敵人以目前裝備／基礎能力生成，不含 VIP 與戰鬥專精；玩家套用本次 GM 測試 VIP 與測試專精。三戰殘血連續，每個新敵人可重新觸發先制。GM 測試不扣副本次數、不給 VIP 積分、不修改正式角色資料。</div>${assessment?`<div class="notice" style="margin-top:10px;border-left-color:${qualified?"#6eaa78":"#d0ad63"}"><b>晉升判定：${s.clearCount} / 500（${pct(s.clearCount,500)}%）・${qualified?"達標":"未達標"}</b><div class="muted">正式門檻為高難 500 次中至少 450 次全通。</div></div>`:""}<div class="stats" style="margin-top:10px;grid-template-columns:repeat(auto-fit,minmax(135px,1fr))"><div class="stat">第1戰通過<b>${pct(s.wins[0],s.runs)}%</b></div><div class="stat">第2戰到達<b>${pct(s.reached[1],s.runs)}%</b></div><div class="stat">第2戰條件通過<b>${conditional(1)}%</b></div><div class="stat">第3戰到達<b>${pct(s.reached[2],s.runs)}%</b></div><div class="stat">第3戰條件通過<b>${conditional(2)}%</b></div><div class="stat">全通率<b>${pct(s.clearCount,s.runs)}%</b></div><div class="stat">平均積分<b>${s.avgPoints}</b></div><div class="stat">全通平均剩餘 HP<b>${s.avgClearHp}%</b></div><div class="stat">平均總回合<b>${s.avgTurns}</b></div></div></div>`;
 }
 ...
 window.gmArena5RunPromotion=function(){
  ...
  const box=document.getElementById("gmArenaTestResult");if(box)box.innerHTML=arenaGm5Result;
  if(button){button.disabled=false;button.textContent="500 次晉升評估（高難）";}
 };
 function arenaGmBody(){
  return `<div class="muted gm-hub-note">可直接測試 Rank1～Rank${Math.max(1,WORLD_REGIONS?.length||1)}，不受正式角色主線解鎖與目前競技場階級限制。敵人不含 VIP／專精；玩家套用上方 GM 測試 VIP 與測試專精。</div><div class="controls" style="align-items:end"><label>競技場階級<br><select id="gmArenaRank5" class="btn">${rankOptions()}</select></label><label>難度<br><select id="gmArenaDifficulty5" class="btn">${difficultyOptions()}</select></label><button id="gmArenaRun100Btn5" class="btn blue" onclick="gmArena5Run100()">100 次完整三連戰</button><button id="gmArenaRun500Btn5" class="btn gm-create" onclick="gmArena5RunPromotion()">500 次晉升評估（高難）</button></div><div class="muted" style="margin-top:8px">500 次晉升評估固定使用高難；450 / 500 以上視為達標。此處只做沙盒測試，不會替正式角色取得晉升資格。</div><div id="gmArenaTestResult" style="margin-top:12px">${arenaGm5Result}</div>`;
 }
