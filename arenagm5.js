(function(){
 ...
 function arenaGmBody(){
  return `<div class="muted gm-hub-note">可直接測試全部 10 個競技場，不受正式角色已解鎖競技場數或主線區域限制。正式玩家則從地球戰爭競技場開始逐個解鎖。</div><div class="controls" style="align-items:end"><label>競技場<br><select id="gmArenaRank5" class="btn">${rankOptions()}</select></label><label>難度<br><select id="gmArenaDifficulty5" class="btn">${difficultyOptions()}</select></label><button id="gmArenaRun100Btn5" class="btn blue" onclick="gmArena5Run100()">100 次完整三連戰</button><button id="gmArenaRun500Btn5" class="btn gm-create" onclick="gmArena5RunPromotion()">500 次戰力評估（高難）</button></div><div class="muted" style="margin-top:8px">500 次戰力評估固定使用高難；450 / 500 以上視為戰力通過。正式玩家還必須解鎖下一競技場對應主線區域，才可開啟下一個競技場。</div><div id="gmArenaTestResult" style="margin-top:12px">${arenaGm5Result}</div>`;
 }
