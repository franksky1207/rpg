(function(){
 ...
 function resultHtml(s,assessment=false){
  ...
  return `<div class="notice"><div class="gm-test-summary"><div class="gm-test-summary-title">${venueName(s.rank)}・${positionLabel(s.positionId)}算法・${s.runs} 次完整三連戰</div><div class="gm-test-summary-line">${vipText}</div><div class="gm-test-summary-line">${specText}</div></div><div class="muted gm-test-context">HP／ATK／DEF 依第 ${s.rank} 個競技場的階層倍率；暴擊、閃避、特性與積分模板依本次位置算法。敵人不含 VIP／戰鬥專精，玩家套用 GM 測試 VIP 與測試專精。</div>${assessment?`<div class="notice" style="margin-top:10px;border-left-color:${qualified?"#6eaa78":"#d0ad63"}"><b>正式戰力評估：${s.clearCount} / 500（${pct(s.clearCount,500)}%）・${qualified?"通過":"未通過"}</b><div class="muted">第1個競技場正式評估用左位（低），第2個用中位（中），第3個起目前最高競技場都位於右側，使用右位（高）。正式解鎖下一個仍需對應主線區域已開。</div></div>`:""}<div class="stats" style="margin-top:10px;grid-template-columns:repeat(auto-fit,minmax(135px,1fr))"><div class="stat">第1戰通過<b>${pct(s.wins[0],s.runs)}%</b></div><div class="stat">第2戰到達<b>${pct(s.reached[1],s.runs)}%</b></div><div class="stat">第2戰條件通過<b>${conditional(1)}%</b></div><div class="stat">第3戰到達<b>${pct(s.reached[2],s.runs)}%</b></div><div class="stat">第3戰條件通過<b>${conditional(2)}%</b></div><div class="stat">全通率<b>${pct(s.clearCount,s.runs)}%</b></div><div class="stat">平均積分<b>${s.avgPoints}</b></div><div class="stat">全通平均剩餘 HP<b>${s.avgClearHp}%</b></div><div class="stat">平均總回合<b>${s.avgTurns}</b></div></div></div>`;
 }
 ...
 function arenaGmBody(){
  return `<div class="muted gm-hub-note">可直接測試全部 10 個正式區域競技場。競技場名稱固定依主線區域；位置算法只控制暴擊／閃避／特性／積分模板，HP／ATK／DEF 仍依競技場實際階層。</div><div class="controls" style="align-items:end"><label>競技場<br><select id="gmArenaRank5" class="btn">${rankOptions()}</select></label><label>位置算法<br><select id="gmArenaPosition5" class="btn">${positionOptions()}</select></label><button id="gmArenaRun100Btn5" class="btn blue" onclick="gmArena5Run100()">100 次完整三連戰</button><button id="gmArenaRun500Btn5" class="btn gm-create" onclick="gmArena5RunPromotion()">500 次正式戰力評估</button></div><div class="muted" style="margin-top:8px">100次測試可自由指定左／中／右位置算法；500次正式評估會自動使用該競技場在正常解鎖流程中作為「目前最高競技場」時的位置算法。</div><div id="gmArenaTestResult" style="margin-top:12px">${arenaGm5Result}</div>`;
 }
