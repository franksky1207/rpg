(function(){
 ...
 function arenaProgress(){
  ...
  const highest=clampRank(d?.arena?.highestArenaUnlocked||d?.arena?.rank||1),start=Math.max(1,highest-2),visible=[];
 ...
 function positionIndexForRank(rank){
  const r=clampRank(rank),visible=arenaProgress().visibleRanks||[],idx=visible.indexOf(r);
  if(idx>=0)return Math.max(0,Math.min(2,idx));
  const highest=clampRank(arenaProgress().highestArenaUnlocked||r),start=Math.max(1,highest-2);
 ...
 function normalizeArenaPhysicalStats(enemy){
  ...
  const idx=Math.max(0,Math.min(2,Math.floor(Number(enemy.arenaStage)||0))),profile=PHYSICAL_STAGE_PROFILE[idx]||PHYSICAL_STAGE_PROFILE[0];
