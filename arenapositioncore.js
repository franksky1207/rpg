(function(){
 ...
 let activePointConfig=null;
 let pointStageCursor=0;
 let awardedPoints=0;
 ...
 if(typeof baseRunCombatCore==="function")window.runCombatCore=function(player,enemy,startHp=null,options={}){normalizeArenaPhysicalStats(enemy);return baseRunCombatCore(player,enemy,startHp,options);};

 const baseBuildArenaEnemyForTest=window.buildArenaEnemyForTest;
 if(typeof baseBuildArenaEnemyForTest==="function")window.buildArenaEnemyForTest=function(difficultyId,stageIndex,stats=null,level=null,rank=null){return normalizeArenaPhysicalStats(baseBuildArenaEnemyForTest(difficultyId,stageIndex,stats,level,rank));};

 const baseGetArenaDifficultyConfigs=window.getArenaDifficultyConfigs;
 if(typeof baseGetArenaDifficultyConfigs==="function")window.getArenaDifficultyConfigs=function(rank=null){return baseGetArenaDifficultyConfigs(rank).map(cfg=>{const pc=pointConfig(cfg.rank||rank||1,cfg.id);return {...cfg,totalPoints:pc.totalPoints,stagePoints:pc.stagePoints};});};

 const baseAddDungeonPoints=window.addDungeonPoints;
 if(typeof baseAddDungeonPoints==="function")window.addDungeonPoints=function(amount){
  const run=typeof getActiveDungeonRun==="function"?getActiveDungeonRun():null;
  if(run?.mode!=="arena"||!activePointConfig)return baseAddDungeonPoints(amount);
  const idx=Math.max(0,Math.min(2,pointStageCursor));
  const corrected=Math.floor(Number(activePointConfig.stagePoints[idx])||0);
  pointStageCursor=(idx+1)%3;
  awardedPoints+=corrected;
  return baseAddDungeonPoints(corrected);
 };

 window.prepareArenaPointRun=function(rank,difficultyId){
  activePointConfig=pointConfig(rank,difficultyId);
  pointStageCursor=0;
  awardedPoints=0;
  return {...activePointConfig,stagePoints:activePointConfig.stagePoints.slice()};
 };
 window.getArenaAwardedPoints=function(){return awardedPoints;};
 ...
 function assessmentSignature(rank,difficultyId){
  ...
  return JSON.stringify({rank:clampRank(rank),positionDifficulty:difficultyId,level:typeof clampGameLevel==="function"?clampGameLevel(state?.level):Math.max(1,Math.floor(Number(state?.level)||1)),base:{hp:base.hp,atk:base.atk,def:base.def,crit:base.crit,dodge:base.dodge},vip,spec:combatSpecSnapshot()});
 }
 ...
 function simulateFullRun(rank,difficultyId,baseStats,playerStats){let hp=playerStats.hp;for(let stage=0;stage<3;stage++){const enemy=window.buildArenaEnemyForTest(difficultyId,stage,baseStats,state.level,rank);if(!enemy)return false;const result=window.runCombatCore(playerStats,enemy,hp,{logs:false});hp=result.hp;if(!result.win)return false;}return true;}
 function assessmentStatus(){
  ...
  const maxRank=Math.max(1,Array.isArray(WORLD_REGIONS)&&WORLD_REGIONS.length?WORLD_REGIONS.length:1),unlockedCap=typeof getArenaUnlockedRankCap==="function"?clampRank(getArenaUnlockedRankCap()):rank;
 ...
 window.assessArenaPromotion=function(){
  ...
  let clears=0;for(let i=0;i<ASSESS_RUNS;i++)if(simulateFullRun(rank,difficultyId,base,player))clears++;
  ...
  save(false);if(typeof render==="function")render();return {...assessmentStatus(),reason:arena.promotionReady?"qualified":"not-qualified"};
 };
