(function(){
 ...
 try{
  const probe={marks:{entries:{}},secondWorld:{calamities:Array.from({length:10},()=>({trueKills:0}))},dungeon:{mirror:{history:{bestWins:17}}},titles:{version:1,unlocked:[],equipped:null,pendingNotice:null}};
  ...
  probe.secondWorld.calamities[0].trueKills=1;
  probe.secondWorld.calamities[4].trueKills=7;
  ...
  if(JSON.stringify(probe.titles.unlocked)!==JSON.stringify(expected))fail("TITLE_BACKFILL_UNION","銀河／宇宙／鏡像靜默補發結果或排序異常",{expected,actual:probe.titles.unlocked});
  probe.titles.unlocked.push(calamityDefs[1].id);
  probe.marks.entries[calamityDefs[1].markId].acquired=false;
  probe.secondWorld.calamities[0].trueKills=0;
 ...
 try{
  const target={marks:{entries:{}},secondWorld:{calamities:Array.from({length:10},()=>({trueKills:0}))},dungeon:{mirror:{history:{bestWins:0}}},titles:{version:1,unlocked:[],equipped:null,pendingNotice:null}};
 ...
 try{
  const target={marks:{entries:{}},secondWorld:{calamities:Array.from({length:10},()=>({trueKills:0}))},dungeon:{mirror:{history:{bestWins:18}}},titles:{version:1,unlocked:[],equipped:null,pendingNotice:null}};
  calamityDefs.forEach(def=>{target.marks.entries[def.markId]={acquired:false,level:0,progress:0};});
