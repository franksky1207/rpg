(function(){
 ...
 if(Number(window.SPECIALIZATION_WORLD_SEMANTICS_VERSION)!==1||Number(window.SPECIALIZATION_PLAYER_WORLD_UI_VERSION)!==1||Number(window.SPECIALIZATION_GM_WORLD_SEMANTICS_VERSION)!==1||Number(window.GM_SPECIALIZATION_FORMAL_RANGE_VERSION)!==1||Number(window.GM_SPECIALIZATION_TEST_RANGE_VERSION)!==1||Number(window.SPECIALIZATION_WORLD_INTEGRITY_VERSION)!==1||Number(window.SPECIALIZATION_FORMAL_STATE_INTEGRITY_VERSION)!==1||typeof window.specializationWorldSemantics!=="function"||typeof window.specializationWorldEconomySummary!=="function"||typeof window.specializationPercentForLevel!=="function"||typeof window.specializationFormalStateIssues!=="function"||typeof window.specializationFormalStateValid!=="function")fail("SPECIALIZATION_WORLD_SEMANTICS","專精玩家／GM 世界感知語意與正式資料 invariant owner 未完整載入",{semantics:window.SPECIALIZATION_WORLD_SEMANTICS_VERSION,playerUi:window.SPECIALIZATION_PLAYER_WORLD_UI_VERSION,gm:window.SPECIALIZATION_GM_WORLD_SEMANTICS_VERSION,formalRange:window.GM_SPECIALIZATION_FORMAL_RANGE_VERSION,testRange:window.GM_SPECIALIZATION_TEST_RANGE_VERSION,integrity:window.SPECIALIZATION_WORLD_INTEGRITY_VERSION,formalState:window.SPECIALIZATION_FORMAL_STATE_INTEGRITY_VERSION});
 try{
  ...
  if(Number(window.SECOND_WORLD_STATE_PRESERVE_UNKNOWN_VERSION)!==1||Number(window.SECOND_WORLD_ENTRY_PURE_READ_VERSION)!==1||Number(window.SECOND_WORLD_CIVILIZATION_RECONCILIATION_VERSION)!==1||typeof window.secondWorldCivilizationFloorFromCalamities!=="function")fail("SECOND_WORLD_STATE_SAFETY","第二世界 state preservation／純讀取／文明 reconciliation owner 未完整載入",{preserve:window.SECOND_WORLD_STATE_PRESERVE_UNKNOWN_VERSION,pureRead:window.SECOND_WORLD_ENTRY_PURE_READ_VERSION,reconcile:window.SECOND_WORLD_CIVILIZATION_RECONCILIATION_VERSION});
