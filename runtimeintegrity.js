(function(){
 ...
 const retiredGmCombatApis=["gmMapMonsterTestHtml","getMapMonsterGmTestHtml","gmStartMapMonsterTest","getSecondWorldBossGmSelection","getSecondWorldBossGmRegionOptions","getSecondWorldBossGmOptions","gmStartSecondWorldBossTest","gmArenaTestHtml","getArenaGmTestHtml","gmSimulateArena","refreshArenaGm5"];
 retiredGmCombatApis.forEach(name=>{if(typeof window[name]!=="undefined")fail("LEGACY_GM_COMBAT_API",`已退休 GM 戰鬥測試 API ${name} 不應再存在`);});
 if(Number(window.GM_ARENA_LEGACY_INJECTION_RETIRED_VERSION)!==1||Number(window.GM_LEGACY_MAP_ARENA_TESTS_RETIRED_VERSION)!==1||Number(window.GM_HUB_LEGACY_COMBAT_RENDERERS_RETIRED_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_LEGACY_FALLBACK_RETIRED_VERSION)!==1)fail("GM_LEGACY_COMBAT_RETIREMENT","GM 舊地圖／競技場測試路徑尚未完整退休",{arenaInjection:window.GM_ARENA_LEGACY_INJECTION_RETIRED_VERSION,dungeon:window.GM_LEGACY_MAP_ARENA_TESTS_RETIRED_VERSION,hub:window.GM_HUB_LEGACY_COMBAT_RENDERERS_RETIRED_VERSION,benchmark:window.GM_POWER_BENCHMARK_LEGACY_FALLBACK_RETIRED_VERSION});
 ...
 if(Number(window.PLAYER_TITLE_CATALOG_VERSION)!==3||Number(window.PLAYER_TITLE_CANONICAL_CATALOG_VERSION)!==2||Number(window.PLAYER_TITLE_UNIFIED_DEFS_VERSION)!==1||!Array.isArray(window.CIVILIZATION_PLAYER_TITLE_DEFS)||window.CIVILIZATION_PLAYER_TITLE_DEFS.length!==10||!Array.isArray(window.UNIVERSE_CALAMITY_PLAYER_TITLE_DEFS)||window.UNIVERSE_CALAMITY_PLAYER_TITLE_DEFS.length!==10||!Array.isArray(window.MIRROR_PLAYER_TITLE_DEFS)||window.MIRROR_PLAYER_TITLE_DEFS.length!==6||!Array.isArray(window.PLAYER_TITLE_DEFS)||window.PLAYER_TITLE_DEFS.length!==26||window.PLAYER_TITLE_DEFS!==window.PLAYER_TITLE_CATALOG_DEFS||window.PLAYER_TITLE_DEFS!==window.PLAYER_TITLE_ALL_DEFS)fail("PLAYER_TITLE_CORE","玩家稱號單一正式 catalog 應為銀河10＋宇宙10＋鏡像6，共26個",{catalog:window.PLAYER_TITLE_CATALOG_VERSION,canonical:window.PLAYER_TITLE_CANONICAL_CATALOG_VERSION,unified:window.PLAYER_TITLE_UNIFIED_DEFS_VERSION,calamity:window.CIVILIZATION_PLAYER_TITLE_DEFS?.length,universe:window.UNIVERSE_CALAMITY_PLAYER_TITLE_DEFS?.length,mirror:window.MIRROR_PLAYER_TITLE_DEFS?.length,total:window.PLAYER_TITLE_DEFS?.length});
 if(Number(window.PLAYER_TITLE_INTEGRITY_VERSION)!==9||window.PLAYER_TITLE_INTEGRITY?.passed!==true)fail("PLAYER_TITLE_INTEGRITY","玩家稱號最終26稱號 integrity 未通過",window.PLAYER_TITLE_INTEGRITY?.errors||null);
 if(Number(window.GM_PLAYER_TITLE_PREVIEW_INTEGRITY_VERSION)!==2||window.GM_PLAYER_TITLE_PREVIEW_INTEGRITY_REPORT?.passed!==true)fail("PLAYER_TITLE_GM_PREVIEW_INTEGRITY","GM 26稱號預覽 integrity 未通過",window.GM_PLAYER_TITLE_PREVIEW_INTEGRITY_REPORT?.errors||null);
 if(Number(window.PLAYER_TITLE_UNIVERSE_VISUAL_INTEGRITY_VERSION)!==2||window.PLAYER_TITLE_UNIVERSE_VISUAL_INTEGRITY_REPORT?.passed!==true)fail("PLAYER_TITLE_UNIVERSE_VISUAL_INTEGRITY","宇宙／鏡像稱號顯示 integrity 未通過",window.PLAYER_TITLE_UNIVERSE_VISUAL_INTEGRITY_REPORT?.errors||null);
 ...
 if(Number(window.SECOND_WORLD_CALAMITY_COMBAT_VERSION)!==1||Number(window.SECOND_WORLD_CALAMITY_SETTLEMENT_VERSION)!==1||Number(window.SECOND_WORLD_CALAMITY_CONTINUOUS_VERSION)!==1||Number(window.SECOND_WORLD_CALAMITY_TITLE_FIRST_KILL_VERSION)!==2||Number(window.SECOND_WORLD_CALAMITY_UI_VERSION)!==2||Number(window.SECOND_WORLD_CALAMITY_PLAYER_SEMANTICS_VERSION)!==1||Number(window.SECOND_WORLD_CALAMITY_APPEARANCE_NOTICE_VERSION)!==2||Number(window.SECOND_WORLD_CALAMITY_UI_INTEGRITY_VERSION)!==1||window.SECOND_WORLD_CALAMITY_UI_INTEGRITY?.passed!==true||typeof window.runSecondWorldCalamityBattle!=="function"||typeof window.runSecondWorldCalamityContinuous!=="function"||typeof window.secondWorldCivilizationCalamityPageHtml!=="function")fail("SECOND_WORLD_CALAMITY_BATCH3","第二世界文明災厄戰鬥／首殺稱號／玩家 UI／現身通知未完整載入",{combat:window.SECOND_WORLD_CALAMITY_COMBAT_VERSION,settlement:window.SECOND_WORLD_CALAMITY_SETTLEMENT_VERSION,continuous:window.SECOND_WORLD_CALAMITY_CONTINUOUS_VERSION,title:window.SECOND_WORLD_CALAMITY_TITLE_FIRST_KILL_VERSION,ui:window.SECOND_WORLD_CALAMITY_UI_VERSION,semantics:window.SECOND_WORLD_CALAMITY_PLAYER_SEMANTICS_VERSION,notice:window.SECOND_WORLD_CALAMITY_APPEARANCE_NOTICE_VERSION,integrity:window.SECOND_WORLD_CALAMITY_UI_INTEGRITY});
 if(Number(window.CIVILIZATION_LEVEL_INTEGRITY_VERSION)!==1||window.CIVILIZATION_LEVEL_INTEGRITY_REPORT?.passed!==true)fail("CIVILIZATION_LEVEL_INTEGRITY","文明等級專屬 Integrity 未通過",window.CIVILIZATION_LEVEL_INTEGRITY_REPORT?.errors||null);
 if(Number(window.CHARACTER_WORLD_UI_VERSION)!==1||Number(window.CHARACTER_CIVILIZATION_UI_VERSION)!==1||typeof window.characterWorldSnapshot!=="function")fail("CHARACTER_CIVILIZATION_UI","角色頁文明等級世界感知 UI owner 未完整載入",{worldUi:window.CHARACTER_WORLD_UI_VERSION,civilizationUi:window.CHARACTER_CIVILIZATION_UI_VERSION,snapshot:typeof window.characterWorldSnapshot});
 ...
 if(Number(window.SPECIALIZATION_WORLD_SEMANTICS_VERSION)!==1||Number(window.SPECIALIZATION_PLAYER_WORLD_UI_VERSION)!==1||Number(window.SPECIALIZATION_GM_WORLD_SEMANTICS_VERSION)!==1||Number(window.GM_SPECIALIZATION_FORMAL_RANGE_VERSION)!==1||Number(window.GM_SPECIALIZATION_TEST_RANGE_VERSION)!==1||Number(window.GM_SPECIALIZATION_ATOMIC_MUTATION_VERSION)!==1||Number(window.SPECIALIZATION_ATOMIC_UPGRADE_VERSION)!==1||Number(window.SPECIALIZATION_WORLD_INTEGRITY_VERSION)!==1||Number(window.SPECIALIZATION_FORMAL_STATE_INTEGRITY_VERSION)!==1||typeof window.specializationWorldSemantics!=="function"||typeof window.specializationWorldEconomySummary!=="function"||typeof window.specializationPercentForLevel!=="function"||typeof window.specializationFormalStateIssues!=="function"||typeof window.specializationFormalStateValid!=="function")fail("SPECIALIZATION_WORLD_SEMANTICS","專精玩家／GM 世界感知語意、atomic mutation 與正式資料 invariant owner 未完整載入",{semantics:window.SPECIALIZATION_WORLD_SEMANTICS_VERSION,playerUi:window.SPECIALIZATION_PLAYER_WORLD_UI_VERSION,gm:window.SPECIALIZATION_GM_WORLD_SEMANTICS_VERSION,formalRange:window.GM_SPECIALIZATION_FORMAL_RANGE_VERSION,testRange:window.GM_SPECIALIZATION_TEST_RANGE_VERSION,gmAtomic:window.GM_SPECIALIZATION_ATOMIC_MUTATION_VERSION,playerAtomic:window.SPECIALIZATION_ATOMIC_UPGRADE_VERSION,integrity:window.SPECIALIZATION_WORLD_INTEGRITY_VERSION,formalState:window.SPECIALIZATION_FORMAL_STATE_INTEGRITY_VERSION});
 try{
  ...
  const invalidUniverseSpec={secondWorld:{entered:true},specializations:{...full,training:59}};
  const validUniverseSpec={secondWorld:{entered:true},specializations:{...full}};
  const galaxySpec={secondWorld:{entered:false},specializations:{...full,training:0}};
  const invalidIssues=window.specializationFormalStateIssues(invalidUniverseSpec);
  if(!invalidIssues.some(row=>row?.code==="LEVEL_BELOW_FORMAL_MIN"&&row?.key==="training"&&row?.value===59))fail("SPECIALIZATION_FORMAL_UNIVERSE_DETECT","宇宙正式專精低於 Lv60 的異常未被偵測",invalidIssues);
  if(window.specializationFormalStateValid(validUniverseSpec)!==true||window.specializationFormalStateValid(galaxySpec)!==true)fail("SPECIALIZATION_FORMAL_VALID","專精銀河／宇宙正式範圍判定異常",{validUniverse:window.specializationFormalStateIssues(validUniverseSpec),galaxy:window.specializationFormalStateIssues(galaxySpec)});
  if(typeof state!=="undefined"&&state?.secondWorld?.entered===true){
   const currentSpecIssues=window.specializationFormalStateIssues(state);
  ...
  if(Number(window.SECOND_WORLD_STATE_PRESERVE_UNKNOWN_VERSION)!==1||Number(window.SECOND_WORLD_ENTRY_PURE_READ_VERSION)!==1||Number(window.SECOND_WORLD_CIVILIZATION_RECONCILIATION_VERSION)!==1||Number(window.SECOND_WORLD_CALAMITY_STRUCTURE_OWNER_VERSION)!==1||Number(window.SECOND_WORLD_CALAMITY_NORMALIZATION_OWNER_VERSION)!==3||Number(window.SECOND_WORLD_CALAMITY_ROW_IDENTITY_VERSION)!==1||Number(window.SECOND_WORLD_CALAMITY_COMPLETION_SEMANTICS_VERSION)!==1||typeof window.secondWorldCivilizationFloorFromCalamities!=="function")fail("SECOND_WORLD_STATE_SAFETY","第二世界 state preservation／純讀取／文明 reconciliation／災厄 ID normalization owner 未完整載入",{preserve:window.SECOND_WORLD_STATE_PRESERVE_UNKNOWN_VERSION,pureRead:window.SECOND_WORLD_ENTRY_PURE_READ_VERSION,reconcile:window.SECOND_WORLD_CIVILIZATION_RECONCILIATION_VERSION,structure:window.SECOND_WORLD_CALAMITY_STRUCTURE_OWNER_VERSION,calamityNormalize:window.SECOND_WORLD_CALAMITY_NORMALIZATION_OWNER_VERSION,rowIdentity:window.SECOND_WORLD_CALAMITY_ROW_IDENTITY_VERSION,completion:window.SECOND_WORLD_CALAMITY_COMPLETION_SEMANTICS_VERSION});
 ...
 if(typeof window.getArenaAssessmentSignature==="function"){
  try{
   ...
   if(state?.secondWorld?.entered===true&&Number(state?.level)>500&&Number(signature?.level)!==Math.floor(Number(state.level)))fail("SECOND_WORLD_ARENA_ASSESSMENT_LEVEL","宇宙競技場評估簽章不得把玩家等級壓回 500",{playerLevel:state.level,signatureLevel:signature?.level});
 ...
 if(Number(window.DUNGEON_UNIVERSE_ARENA_UI_VERSION)!==1||Number(window.ARENA_UNIVERSE_PLAYER_FLOW_UI_VERSION)!==1||Number(window.GAME_GUIDE_ARENA_WORLD_VERSION)!==1||Number(window.SECOND_WORLD_ARENA_LEVEL_CLAMP_VERSION)!==1||Number(window.SECOND_WORLD_ARENA_ASSESSMENT_LEVEL_VERSION)!==1)fail("SECOND_WORLD_ARENA_UI_INTEGRATION","第二世界競技場 UI／說明／等級語意未完整載入",{dungeonUi:window.DUNGEON_UNIVERSE_ARENA_UI_VERSION,playerUi:window.ARENA_UNIVERSE_PLAYER_FLOW_UI_VERSION,guide:window.GAME_GUIDE_ARENA_WORLD_VERSION,levelClamp:window.SECOND_WORLD_ARENA_LEVEL_CLAMP_VERSION,assessmentLevel:window.SECOND_WORLD_ARENA_ASSESSMENT_LEVEL_VERSION});
 ...
 if(Number(window.GAME_GUIDE_VERSION)!==18||Number(window.GAME_GUIDE_CALAMITY_WORLD_VERSION)!==1||Number(window.GAME_GUIDE_BOUNTY_WORLD_VERSION)!==1||Number(window.GAME_GUIDE_ARENA_WORLD_VERSION)!==1)fail("GUIDE_VERSION",`遊戲說明應為 V18 並具備文明災厄／懸賞／競技場雙紀元語意，實際 ${window.GAME_GUIDE_VERSION}`);
 ...
 if(Number(window.CALAMITY_CORE_VERSION)!==1||Number(window.CALAMITY_COMBAT_RULE_VERSION)!==3||Number(window.CALAMITY_HP_RESTORE_OWNER_VERSION)!==1||Number(window.CALAMITY_MAXED_REPLAY_HP_VERSION)!==1||Number(window.COMBAT_PERSISTENT_ENEMY_HP_VERSION)!==1)fail("CALAMITY_CORE_VERSION","文明災厄 Core／滿印記重打 HP／持久敵方 HP 版本異常",{core:window.CALAMITY_CORE_VERSION,rules:window.CALAMITY_COMBAT_RULE_VERSION,restoreOwner:window.CALAMITY_HP_RESTORE_OWNER_VERSION,maxedReplayHp:window.CALAMITY_MAXED_REPLAY_HP_VERSION,persistentHp:window.COMBAT_PERSISTENT_ENEMY_HP_VERSION});
 if(Number(window.CALAMITY_RUN_VERSION)!==1||Number(window.CALAMITY_CONTINUOUS_RULE_VERSION)!==5||Number(window.CALAMITY_MAXED_MARK_CONTINUOUS_STOP_VERSION)!==1)fail("CALAMITY_RUN_VERSION","文明災厄 runtime／滿印記連戰停止規則版本異常",{run:window.CALAMITY_RUN_VERSION,rules:window.CALAMITY_CONTINUOUS_RULE_VERSION,maxedStop:window.CALAMITY_MAXED_MARK_CONTINUOUS_STOP_VERSION});
 if(Number(window.CALAMITY_UI_VERSION)!==3||Number(window.CALAMITY_MAXED_REPLAY_SINGLE_ONLY_UI_VERSION)!==1||Number(window.CALAMITY_MINIMAL_MODE_VERSION)!==1||Number(window.CALAMITY_BATTLE_VIEW_VERSION)!==1||Number(window.CALAMITY_OUTER_PACING_VERSION)!==1)fail("CALAMITY_UI_VERSION","文明災厄 UI／battleView／極簡模式／Outer Pacing 接線異常",{ui:window.CALAMITY_UI_VERSION,battleView:window.CALAMITY_BATTLE_VIEW_VERSION,minimal:window.CALAMITY_MINIMAL_MODE_VERSION,outer:window.CALAMITY_OUTER_PACING_VERSION});
 if(Number(window.GM_CALAMITY_TEST_VERSION)!==1||Number(window.GM_MARK_MANAGEMENT_VERSION)!==1||Number(window.GM_MARK_CONFIG_OWNER_VERSION)!==1||Number(window.GM_CALAMITY_FULL_KILL_SAFETY_LIMIT)!==100000||Number(window.GM_CALAMITY_LEGACY_TITLE_PREVIEW_RETIRED_VERSION)!==1)fail("CALAMITY_GM_VERSION","文明災厄／印記 GM 或舊稱號預覽退休狀態異常",{calamity:window.GM_CALAMITY_TEST_VERSION,marks:window.GM_MARK_MANAGEMENT_VERSION,configOwner:window.GM_MARK_CONFIG_OWNER_VERSION,safety:window.GM_CALAMITY_FULL_KILL_SAFETY_LIMIT,titlePreviewRetired:window.GM_CALAMITY_LEGACY_TITLE_PREVIEW_RETIRED_VERSION});
 ...
 if(typeof window.gmHubRegisteredSectionIds==="function"){
  ...
  const testExpected=["player-ability-test","power-benchmark-test","player-title-preview","gm-story-test"];
 ...
 if(Number(window.GM_PLAYER_ABILITY_TEST_GROUP_VERSION)!==2||typeof window.gmPlayerAbilityTestHtml!=="function")fail("GM_PLAYER_ABILITY_TEST_GROUP","角色能力測試大區未完整載入",{version:window.GM_PLAYER_ABILITY_TEST_GROUP_VERSION,renderer:typeof window.gmPlayerAbilityTestHtml});
 if(Number(window.GM_CIVILIZATION_ATOMIC_MUTATION_VERSION)!==1)fail("GM_CIVILIZATION_ATOMIC_MUTATION","GM 文明等級正式 mutation 必須具備 save failure rollback",window.GM_CIVILIZATION_ATOMIC_MUTATION_VERSION);
 if(Number(window.GM_SECOND_WORLD_CALAMITY_ATOMIC_MUTATION_VERSION)!==1)fail("GM_SECOND_WORLD_CALAMITY_ATOMIC_MUTATION","GM 宇宙文明災厄正式 mutation 必須具備 save failure rollback",window.GM_SECOND_WORLD_CALAMITY_ATOMIC_MUTATION_VERSION);
 if(Number(window.GM_TEST_STATE_VERSION)!==2||Number(window.GM_TEST_CHARACTER_SANDBOX_VERSION)!==1||Number(window.GM_TEST_CHARACTER_EQUIPMENT_MODE_VERSION)!==1||typeof window.gmTestCharacterSnapshot!=="function"||typeof window.gmSetTestWorld!=="function"||typeof window.gmSetTestLevel!=="function"||typeof window.gmTestCharacterBaseHtml!=="function"||Number(window.GM_ENHANCEMENT_TEST_PIPELINE_VERSION)!==6||Number(window.GM_ENHANCEMENT_TEST_RANGE_VERSION)!==1||Number(window.GM_ENHANCEMENT_HUB_VERSION)!==5||Number(window.GM_ENHANCEMENT_FORMAL_RANGE_VERSION)!==1||Number(window.GM_CIVILIZATION_VERSION)!==1||Number(window.GM_CIVILIZATION_FORMAL_RANGE_VERSION)!==1||Number(window.GM_CIVILIZATION_TEST_RANGE_VERSION)!==1||typeof window.gmTestCivilizationLevelValue!=="function"||typeof window.gmSetTestCivilizationLevel!=="function"||typeof window.gmRefreshTestControls!=="function"||typeof window.gmTestEnhancementSlots!=="function"||typeof window.gmClampTestEnhancementLevel!=="function"||typeof window.gmUseCurrentTestStatus!=="function")fail("GM_TEST_STATE_OWNER","GM 共用測試狀態 owner／強化與文明等級正式／沙盒範圍未完整載入",{stateVersion:window.GM_TEST_STATE_VERSION,pipeline:window.GM_ENHANCEMENT_TEST_PIPELINE_VERSION,testRange:window.GM_ENHANCEMENT_TEST_RANGE_VERSION,hub:window.GM_ENHANCEMENT_HUB_VERSION,formalRange:window.GM_ENHANCEMENT_FORMAL_RANGE_VERSION,civilization:window.GM_CIVILIZATION_VERSION,civilizationFormal:window.GM_CIVILIZATION_FORMAL_RANGE_VERSION,civilizationTest:window.GM_CIVILIZATION_TEST_RANGE_VERSION,refresh:typeof window.gmRefreshTestControls,slots:typeof window.gmTestEnhancementSlots,clamp:typeof window.gmClampTestEnhancementLevel,sync:typeof window.gmUseCurrentTestStatus});
 if(Number(window.PENDING_BLACK_MARKET_FLAG_SEMANTICS_VERSION)!==1||Number(window.PENDING_BLACK_MARKET_WORLD_TRANSITION_VERSION)!==1||Number(window.PENDING_BLACK_MARKET_CURRENT_PHASE_VERSION)!==1||typeof window.pendingBlackMarketActive!=="function"||typeof window.resetPendingBlackMarketForWorldTransition!=="function")fail("BLACK_MARKET_FLAG_SEMANTICS","黑市待觸發旗標的當前紀元語意／跨紀元重置未完整載入",{normalize:window.PENDING_BLACK_MARKET_FLAG_SEMANTICS_VERSION,transition:window.PENDING_BLACK_MARKET_WORLD_TRANSITION_VERSION,currentPhase:window.PENDING_BLACK_MARKET_CURRENT_PHASE_VERSION});
 if(Number(window.GM_TEST_SAVE_ISOLATION_VERSION)!==1||Number(window.GM_TEST_SESSION_ONLY_VERSION)!==1||!Array.isArray(window.GM_TEST_TRANSIENT_STATE_KEYS)||typeof window.cleanupTransientGmTestState!=="function")fail("GM_TEST_SAVE_ISOLATION","GM 測試設定／結果必須只存在頁面工作階段，不得寫入正式存檔",{saveIsolation:window.GM_TEST_SAVE_ISOLATION_VERSION,sessionOnly:window.GM_TEST_SESSION_ONLY_VERSION,keys:window.GM_TEST_TRANSIENT_STATE_KEYS});
 if(Number(window.GM_TEST_BATCH_SYNC_VERSION)!==1||Number(window.GM_MARK_TEST_BATCH_SYNC_VERSION)!==1||Number(window.GM_CIVILIZATION_TEST_BATCH_SYNC_VERSION)!==1)fail("GM_TEST_BATCH_SYNC","GM 正式角色同步應採單次失效／刷新批次流程",{sync:window.GM_TEST_BATCH_SYNC_VERSION,marks:window.GM_MARK_TEST_BATCH_SYNC_VERSION,civilization:window.GM_CIVILIZATION_TEST_BATCH_SYNC_VERSION});
 if(Number(window.GM_TEST_ARCHITECTURE_MANIFEST_VERSION)!==1||typeof window.gmTestArchitectureManifest!=="function")fail("GM_TEST_ARCHITECTURE_MANIFEST","GM 測試架構聚合 manifest 未載入",{version:window.GM_TEST_ARCHITECTURE_MANIFEST_VERSION,api:typeof window.gmTestArchitectureManifest});
 else{const gmArch=window.gmTestArchitectureManifest();if(!gmArch.sessionOnly||!gmArch.batchSync||!gmArch.benchmark||!gmArch.stateIsolation||!gmArch.legacyRetired||gmArch.persistedTransientKeys.length)fail("GM_TEST_ARCHITECTURE_STATUS","GM 測試架構狀態異常",gmArch);}
 if(Number(window.GM_POWER_BENCHMARK_VERSION)!==21||Number(window.GM_POWER_BENCHMARK_GROUP_VERSION)!==2||Number(window.GM_POWER_BENCHMARK_ALL_MODES_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_UNIFIED_SUMMARY_VERSION)!==2||Number(window.GM_POWER_BENCHMARK_LIVE_REFRESH_VERSION)!==1||Number(window.GM_TEST_SYNC_BENCHMARK_REFRESH_VERSION)!==1||Number(window.GM_TEST_SESSION_UI_VERSION)!==1||Number(window.GM_ARENA_SESSION_SETTINGS_VERSION)!==1||Number(window.GM_CALAMITY_SESSION_SETTINGS_VERSION)!==1||Number(window.GM_SECOND_WORLD_CALAMITY_SESSION_SETTINGS_VERSION)!==1||Number(window.GM_TEST_RESULT_INVALIDATION_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_GM_CHARACTER_VERSION)!==1||Number(window.GM_SPECIAL_WORLD_BENCHMARK_VERSION)!==1||Number(window.SPECIAL_WORLD_DROP_OWNER_VERSION)!==2||Number(window.SPECIAL_WEAK_SLOT_CONTEXT_VERSION)!==1||Number(window.GM_SPECIAL_WEAK_SLOT_SANDBOX_VERSION)!==1||Number(window.BOUNTY_TEST_CONTEXT_VERSION)!==1||Number(window.GM_BOUNTY_STATE_ISOLATION_VERSION)!==1||Number(window.ARENA_EXPLICIT_WORLD_CONTEXT_VERSION)!==1||Number(window.GM_ARENA_STATE_ISOLATION_VERSION)!==1||Number(window.GM_SPECIAL_SUMMARY_EXPORT_VERSION)!==1||Number(window.GM_BOUNTY_INDEPENDENT_WORLD_TEST_VERSION)!==1||Number(window.GM_DUNGEON_SUMMARY_EXPORT_VERSION)!==1||Number(window.GM_ARENA_INDEPENDENT_WORLD_TEST_VERSION)!==1||Number(window.GM_ARENA_SUMMARY_EXPORT_VERSION)!==1||Number(window.GM_MIRROR_SHARED_TEST_CHARACTER_VERSION)!==1||Number(window.GM_MIRROR_SUMMARY_EXPORT_VERSION)!==1||Number(window.GM_CALAMITY_TEST_EMBEDDED_VERSION)!==1||Number(window.GM_CALAMITY_SUMMARY_EXPORT_VERSION)!==1||Number(window.GM_SECOND_WORLD_CALAMITY_TEST_EMBEDDED_VERSION)!==1||Number(window.GM_SECOND_WORLD_CALAMITY_SUMMARY_EXPORT_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_ENHANCEMENT_RANGE_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_SPECIALIZATION_WORLD_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_CIVILIZATION_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_CALAMITY_INTEGRATION_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_CALAMITY_VERSION)!==1||Number(window.GM_POWER_BENCHMARK_BATCH_SIZE)!==25||typeof window.gmPowerBenchmarkSetWorld!=="function"||typeof window.gmPowerBenchmarkSelectedEnemy!=="function"||typeof window.gmPowerBenchmarkRunOutput!=="function"||typeof window.gmPowerBenchmarkRunDefense!=="function"||typeof window.gmPowerBenchmarkRunCombat!=="function"||typeof window.gmPowerBenchmarkSetUniverseRegion!=="function"||typeof window.gmPowerBenchmarkSetUniverseBoss!=="function"||typeof window.gmPowerBenchmarkIsBusy!=="function"||typeof window.gmPowerBenchmarkSummaryText!=="function"||typeof window.gmPowerBenchmarkCopySummary!=="function"||typeof window.gmPowerBenchmarkClearAllResults!=="function"||typeof window.gmPowerBenchmarkReset!=="function"||typeof window.gmPowerBenchmarkRunUniverseCombat!=="undefined")fail("GM_POWER_BENCHMARK","GM 戰力基準測試 V21／統一摘要與各模式結果匯出未完整載入",{version:window.GM_POWER_BENCHMARK_VERSION,enhancementRange:window.GM_POWER_BENCHMARK_ENHANCEMENT_RANGE_VERSION,specializationWorld:window.GM_POWER_BENCHMARK_SPECIALIZATION_WORLD_VERSION,civilization:window.GM_POWER_BENCHMARK_CIVILIZATION_VERSION,batch:window.GM_POWER_BENCHMARK_BATCH_SIZE,world:typeof window.gmPowerBenchmarkSetWorld,selectedEnemy:typeof window.gmPowerBenchmarkSelectedEnemy,output:typeof window.gmPowerBenchmarkRunOutput,defense:typeof window.gmPowerBenchmarkRunDefense,combat:typeof window.gmPowerBenchmarkRunCombat,universeRegion:typeof window.gmPowerBenchmarkSetUniverseRegion,universeBoss:typeof window.gmPowerBenchmarkSetUniverseBoss,legacyUniverse:typeof window.gmPowerBenchmarkRunUniverseCombat,busy:typeof window.gmPowerBenchmarkIsBusy});
 if(Number(window.GM_SECOND_WORLD_CALAMITY_VERSION)!==1||Number(window.GM_SECOND_WORLD_CALAMITY_TEST_VERSION)!==1||Number(window.GM_SECOND_WORLD_CALAMITY_MANAGE_RETIRED_VERSION)!==1||Number(window.GM_SECOND_WORLD_CALAMITY_INTEGRITY_VERSION)!==1||window.GM_SECOND_WORLD_CALAMITY_INTEGRITY?.passed!==true||typeof window.gmSecondWorldCalamityTestHtml!=="function")fail("GM_SECOND_WORLD_CALAMITY","宇宙文明災厄 GM 測試／Benchmark 未完整載入，或已退休管理仍有異常",{version:window.GM_SECOND_WORLD_CALAMITY_VERSION,test:window.GM_SECOND_WORLD_CALAMITY_TEST_VERSION,manageRetired:window.GM_SECOND_WORLD_CALAMITY_MANAGE_RETIRED_VERSION,integrity:window.GM_SECOND_WORLD_CALAMITY_INTEGRITY});
 if(Number(window.SECOND_WORLD_CALAMITY_FULL_INTEGRITY_VERSION)!==2||window.SECOND_WORLD_CALAMITY_FULL_INTEGRITY_REPORT?.passed!==true||typeof window.runSecondWorldCalamityFullIntegrity!=="function")fail("SECOND_WORLD_CALAMITY_FULL_INTEGRITY","第二世界文明災厄完整 Integrity V2 未通過",window.SECOND_WORLD_CALAMITY_FULL_INTEGRITY_REPORT);
 ...
 if(Number(window.SECOND_WORLD_SPECIALIZATION_ECONOMY_VERSION)!==1)fail("SECOND_WORLD_SPECIALIZATION_OWNER","宇宙專精 EXP／暗物質／出售正式 owner 未完整載入",{economy:window.SECOND_WORLD_SPECIALIZATION_ECONOMY_VERSION});
 {
  const keys=Array.isArray(window.SPECIALIZATION_KEYS)?window.SPECIALIZATION_KEYS:[];
 ...
 if(Number(window.BOUNTY_UNIVERSE_UI_VERSION)!==1||Number(window.DUNGEON_UNIVERSE_BOUNTY_UI_VERSION)!==1||Number(window.GAME_GUIDE_BOUNTY_WORLD_VERSION)!==1||Number(window.GM_BOUNTY_UNIVERSE_PREVIEW_VERSION)!==1)fail("BOUNTY_UNIVERSE_UI_INTEGRATION","宇宙懸賞 UI／說明／GM 整合未完整載入",{bountyUi:window.BOUNTY_UNIVERSE_UI_VERSION,dungeonUi:window.DUNGEON_UNIVERSE_BOUNTY_UI_VERSION,guide:window.GAME_GUIDE_BOUNTY_WORLD_VERSION,gm:window.GM_BOUNTY_UNIVERSE_PREVIEW_VERSION});
 else{
