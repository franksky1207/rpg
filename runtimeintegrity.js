(function(){
 ...
 if(Number(window.PLAYER_TITLE_STATE_VERSION)!==1||!Array.isArray(window.CIVILIZATION_PLAYER_TITLE_DEFS)||window.CIVILIZATION_PLAYER_TITLE_DEFS.length!==10)fail("PLAYER_TITLE_CORE","玩家稱號核心 V1／10 階定義未正確載入",{version:window.PLAYER_TITLE_STATE_VERSION,count:window.CIVILIZATION_PLAYER_TITLE_DEFS?.length});
 if(Number(window.GM_PLAYER_TITLE_PREVIEW_VERSION)!==1)fail("PLAYER_TITLE_GM_PREVIEW","GM 稱號純預覽 V1 未載入",window.GM_PLAYER_TITLE_PREVIEW_VERSION);
 if(Number(window.PLAYER_TITLE_INTEGRITY_VERSION)!==1||window.PLAYER_TITLE_INTEGRITY?.passed!==true)fail("PLAYER_TITLE_INTEGRITY","玩家稱號專屬 integrity 未通過",window.PLAYER_TITLE_INTEGRITY?.errors||null);
 ...
 if(Number(window.COMBAT_OUTER_PACING_VERSION)!==1||typeof window.combatOuterGapMs!=="function"||Number(window.combatOuterGapMs("main","normal"))!==140||Number(window.combatOuterGapMs("main","elite"))!==220||Number(window.combatOuterGapMs("main","boss"))!==140||Number(window.combatOuterGapMs("void","floor"))!==350||Number(window.combatOuterGapMs("calamity","battle"))!==350)fail("COMBAT_OUTER_PACING_OWNER","戰鬥外場間節奏 owner 異常",{version:window.COMBAT_OUTER_PACING_VERSION});
