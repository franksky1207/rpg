...
function gmSpecialTestVip(){return typeof window.normalizeVipLevel==="function"?window.normalizeVipLevel(window.gmTestVipLevel):Math.max(0,Math.floor(Number(window.gmTestVipLevel)||0));}
...
async function gmStartSpecialBattle(){
 ...
 try{
  ...
  console.error("GM special benchmark failed",error);
  const result=document.getElementById("gmSpecialBatchResult");if(result)result.innerHTML='<div class="notice">特殊怪測試執行失敗，請重新整理後再試。</div>';
 }finally{
  battleBusy=false;
