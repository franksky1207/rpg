(function(){
 ...
 window.specializationLevel=function(key,useTest=false){
  if(!SPECIALIZATION_DEFS[key])return 0;
 ...
 function effectLines(key,lv){
  ...
  if(key==="scavenge")return [`怪物金幣 +${lv*5}%`];
  if(key==="appraisal")return [`裝備售價 +${lv*5}%`];
  if(key==="initiative")return [`第一擊傷害 +${lv*2}%`];
  if(key==="combo")return [`連擊率 ${lv}%`,`追加傷害 50%`];
  if(key==="penetration")return [`穿透率 ${lv}%`,`忽略防禦 25%`];
  if(key==="counter")return [`反擊率 ${lv}%`,`反擊傷害 40%`];
  if(key==="drain")return [`汲取率 ${lv}%`,`回復傷害 10%`];
  return [];
 }
 function specializationCard(key){
  const def=SPECIALIZATION_DEFS[key],lv=formalLevel(key),maxed=lv>=SPECIALIZATION_MAX_LEVEL,cost=maxed?0:specializationUpgradeCost(lv+1),enough=maxed||state.gold>=cost;
