(function(){
 ...
 function effectLines(key,lv){if(key==="training")return [`EXP +${lv*2.5}%`];if(key==="scavenge")return [`${universePhase()?"暗物質":"怪物金幣"} +${lv*2.5}%`];if(key==="appraisal")return [`${universePhase()?"裝備暗物質售價":"裝備售價"} +${lv*2.5}%`];if(key==="initiative")return [`第一擊傷害 +${lv}%`];if(key==="combo")return [`連擊率 ${lv*.5}%`,`追加傷害 50%`];if(key==="penetration")return [`穿透率 ${lv*.5}%`,`忽略防禦 25%`];if(key==="counter")return [`反擊率 ${lv*.5}%`,`反擊傷害 40%`];if(key==="drain")return [`汲取率 ${lv*.5}%`,`回復傷害 10%`];return [];}
 ...
 function specializationPage(){ensureSpecializationState();const universe=universePhase(),goldHtml=universe?"":`<div class="specialization-gold">金幣 <b>${state.gold.toLocaleString()}</b></div>`,guide=universe?`<details class="specialization-guide"><summary>專精說明</summary><div class="specialization-guide-body"><div class="muted">銀河紀元完成的專精會在宇宙紀元持續生效。</div>${SPECIALIZATION_KEYS.map(key=>{const d=SPECIALIZATION_DEFS[key];let desc=d.desc,per=d.perLevel;if(key==="scavenge"){desc="提升宇宙紀元主線取得的暗物質。";per="每級暗物質 +2.5%";}if(key==="appraisal"){desc="提升宇宙紀元裝備出售取得的暗物質。";per="每級裝備暗物質售價 +2.5%";}return `<div class="specialization-guide-row"><b>${d.name}</b><div>${desc}</div><div class="muted">${per}</div></div>`;}).join("")}</div></details>`:specializationGuideHtml();return `<div class="function-page specialization-page"><div class="back-home"><button class="btn back-btn" onclick="go('home')">← 返回主頁</button></div><div class="card specialization-panel"><div class="specialization-title-row"><h2>專精</h2>${goldHtml}</div>${guide}<div class="specialization-grid">${SPECIALIZATION_KEYS.map(specializationCard).join("")}</div></div></div>`;}
