(function(){
 ...
 function showStageSettlement(ctx,action){
  ...
  detail.innerHTML=`<div class="notice"><b>已完成 ${done} / ${total} 場</b><div class="muted" style="margin-top:5px">以下獎勵已正式取得並保留。</div></div><div class="stats" style="margin-top:10px"><div class="stat">EXP<b>+${ctx.totalXp||0}</b></div><div class="stat">戰鬥金幣<b>+${ctx.totalGold||0}</b></div><div class="stat">自動出售<b>+${soldGold}</b></div><div class="stat">金幣合計<b>+${goldTotal}</b></div></div>${upgradeCount?`<div class="notice" style="margin-top:10px"><b>★ 有 ${upgradeCount} 件掉落可提升目前裝備</b></div>`:`<div class="muted" style="margin-top:10px">本階段沒有可提升目前裝備的掉落。</div>`}${stageDropListHtml(ctx)}`;
