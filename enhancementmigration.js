(function(){
 ...
 if(typeof window.effectiveEnhancementCap!=="function"||typeof window.clampEffectiveEnhancementLevel!=="function"||typeof window.enhancementUpgradeCost!=="function")fail("WORLD_AWARE_API","世界感知強化 API 未完整載入");
 else{
  const galaxy={secondWorld:{entered:false}},universe={secondWorld:{entered:true}};
  if(window.effectiveEnhancementCap(galaxy)!==20||window.effectiveEnhancementCap(universe)!==40)fail("WORLD_AWARE_CAP","銀河／宇宙強化 cap 異常",{galaxy:window.effectiveEnhancementCap(galaxy),universe:window.effectiveEnhancementCap(universe)});
 ...
 window.ENHANCEMENT_MIGRATION_WORLD_AWARE_VERSION=1;
