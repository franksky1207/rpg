(function(){
 ...
 function run(){
  ...
  if(Number(window.MINIMAL_MODE_SHARED_API_VERSION)!==1)fail("shared-api-version");
  if(Number(window.VOID_MINIMAL_MODE_HOOK_VERSION)!==1)fail("void-hook-version");

  if(typeof window.openMinimalMode!=="function")fail("shared-open-api");
  ...
  if(typeof window.minimalModeBackgroundPolicy!=="function"||window.minimalModeBackgroundPolicy()!=="follow-gm-background-setting")fail("shared-background-policy");
  ...
  const report={version:VERSION,ok:issues.length===0,issues,sharedApiVersion:Number(window.MINIMAL_MODE_SHARED_API_VERSION)||0,backgroundPolicy:typeof window.minimalModeBackgroundPolicy==="function"?window.minimalModeBackgroundPolicy():null};
