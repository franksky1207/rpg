(function(){
 ...
 function specialEntryHtml(entry,index,total){
  ...
  return `<div class="settlement-special-entry"><div class="settlement-special-name">${prefix}✦ ${name} 挑戰失敗</div><div class="notice" style="margin-top:9px">本次連續戰鬥立即結束。</div>${lost?`<div style="margin-top:9px"><b>遺失裝備</b><div class="item">${itemHtml(lost,true)}${typeof gearAbilityHtml==="function"?gearAbilityHtml(lost,true):""}</div><div class="muted">已移至背包的「遺失裝備贖回」。</div></div>`:`<div class="muted" style="margin-top:9px">本次沒有遺失裝備。</div>`}${result.penalty?.protectedByVip20?`<div class="vip-event">【VIP20】裝備受到保護，本次死亡沒有遺失裝備。</div>`:""}</div>`;
 }
