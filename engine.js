...
function redeemLostGear(i){if(typeof window.isSecondWorldEntered==="function"&&window.isSecondWorldEntered())return {ok:false,reason:"宇宙紀元裝備贖回系統尚未開放。"};let lost=state.lostGear[i];if(!lost)return {ok:false,reason:"找不到這件遺失裝備。"};if(state.gold<lost.cost)return {ok:false,reason:"金幣不足。"};if(!claimLostGearMutation())return {ok:true,ignored:true};state.gold-=lost.cost;state.inventory.push(lost.item);state.lostGear.splice(i,1);save(false);return {ok:true,item:lost.item}}
