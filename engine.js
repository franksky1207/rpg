...
function addItem(it,options={}){if(!it)return {kept:false,sold:0};let slot=state.equipment[it.type],upgrade=equipmentScore(it)>equipmentScore(slot);if(typeof window.isSecondWorldEntered==="function"&&window.isSecondWorldEntered()){state.inventory.push(it);if(upgrade)upgradeDropNoticePending=true;return {kept:true,sold:0}}if(it.q===5||(state.settings.keepUpgrade&&upgrade)){state.inventory.push(it);if(upgrade)upgradeDropNoticePending=true;return {kept:true,sold:0}}if(it.q<=4&&state.settings.autoSell[it.q]){const sold=specializationSellValue(it,options.useTestSpecializations===true);state.gold+=sold;return {kept:false,sold}}state.inventory.push(it);if(upgrade)upgradeDropNoticePending=true;return {kept:true,sold:0}}
