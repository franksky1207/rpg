(function(){
 ...
 function page(){ensure();const universe=typeof window.isSecondWorldEntered==="function"&&window.isSecondWorldEntered();const resourceHtml=universe?'<div class="notice"><b>宇宙紀元高階強化尚未開放。</b><div class="muted" style="margin-top:5px">銀河紀元的基礎／進階強化石已結束使用；既有 +20 強化等級完整保留。</div></div>':`<div class="enhance-resources"><div><span>基礎強化石</span><b>${state.enhancement.basicStones.toLocaleString()}</b></div><div><span>進階強化石</span><b>${state.enhancement.advancedStones.toLocaleString()}</b></div></div>`;return `<div class="function-page enhancement-page"><div class="back-home"><button class="btn back-btn" onclick="go('home')">← 返回主頁</button></div><div class="enhance-shell"><div class="card enhance-summary"><div class="enhance-title"><div><h2>裝備欄位強化</h2><div class="muted">永久提升裝備欄位主能力；更換或遺失裝備不影響強化等級。</div></div><div class="enhance-progress">強化進度 <b>${progress()} / ${ENHANCEMENT_SLOTS.length*ENHANCEMENT_MAX_LEVEL}</b></div></div>${resourceHtml}</div><div class="enhance-grid">${ENHANCEMENT_SLOTS.map(slotCard).join("")}</div></div></div>`;}
 ...
 window.openEnhancementConfirm=function(type){
  if(typeof window.isSecondWorldEntered==="function"&&window.isSecondWorldEntered())return alert("宇宙紀元高階強化尚未開放。");
