...
- 2026-09-19 玩家稱號系統第 2 批：正式首殺發放已接入災厄 settlement。唯一首殺證據仍為既有 `markSettlement.firstAcquisition===true`；`calamitycore.js` 在印記首次取得後、正式 save 前呼叫 `grantPlayerTitleForCalamityFirstKill()`，把對應稱號加入 `titles.unlocked` 並寫入 `pendingNotice`。舊檔 backfill 仍只做靜默補齊，不建立通知。連續討伐規則升為 `CALAMITY_CONTINUOUS_RULE_VERSION=4`：`calamityrun.js` 若本場 `titleSettlement.firstAcquisition===true`，立即以 `title-first-kill` 結束 runtime／background flow，不再開始下一場；單場仍正常以 single-complete 結束。稱號通知由 `calamityui.js` 讀取持久化 `pendingNotice`；前景完成動畫後顯示「獲得稱號」視窗，背景首殺不在隱藏頁面彈窗，回前景或下次可見載入時才顯示；玩家確認後才清除 `pendingNotice` 並存檔。第 2 批仍未加入正式稱號 renderer／角色頁裝備／10 階 CSS／GM 預覽，這些留後續批次。Save Schema 維持 13，災厄數值與印記升級規則未改。

- 2026-09-19 玩家稱號系統第 3 批：完成正式身分 renderer、角色頁稱號選擇與 10 階視覺。唯一 HTML owner 為 `playertitlecore.js -> playerIdentityNameHtml()/playerTitleHtml()`；正式玩家身分 UI 已接入主線狀態卡／主線戰鬥／特殊遭遇／角色頁／懸賞／競技場／虛空／文明災厄／鏡像戰，鏡像對手顯示亦沿用玩家目前稱號，但 combat core 與 log 仍只保存純文字名稱。劇情 `storyui.js`、戰線紀錄 `storyrecordtabs.js` 與設定改名 input 刻意維持純玩家名，不接稱號 HTML。角色頁只有至少 1 個已解鎖稱號時才顯示「稱號」入口；選擇面板只列「不裝備稱號」與已解鎖稱號，未解鎖項目與 X/10 均不顯示，點選後立即存檔／生效。新增 `playertitles.css`：10 階共用一套與一般 UI 不同的 serif 類稱號字型，1～10 依序增加呼吸光、流光、星塵、殘影、能量環、尾跡、多層脈衝、核心扭曲感、壓迫殘影、最終星塵整合；無前後括號／符號裝飾。手機戰鬥名稱允許必要時在稱號與名字之間換行，避免長英文名撐爆；`prefers-reduced-motion` 關閉動畫但保留稱號顏色／字型識別。首殺通知改直接使用同一 `playerTitleHtml()` 真實樣式。第 3 批尚未做 GM 純預覽與正式 title integrity，留第 4 批。Save Schema 13、災厄／印記數值規則不變。

- 2026-09-19 玩家稱號系統第 4 批：完成 GM 純預覽與防回歸收尾。calamitygm.js 新增 GM_PLAYER_TITLE_PREVIEW_VERSION=1 與「稱號預覽」GM 測試區，可直接切換第 1～10 階並使用正式 playerTitleHtml() 顯示真實效果；預覽選擇只保存在 runtime 變數，不寫 state.titles、不改 unlocked/equipped/pendingNotice、不修改印記／災厄，也不呼叫 save。新增 playertitleintegrity.js V1，鎖定 10 個稱號 ID／名稱／階級順序、正式 API、normalization idempotent、舊檔 backfill 靜默且 additive、同一災厄首殺只取得一次、真正首殺建立 pendingNotice、renderer 維持獨立 title/name span，以及 GM 預覽不得改正式 title state 或 localStorage。Runtime Integrity 新增稱號核心／GM preview／專屬 integrity 鎖定，Final Integrity 將 PLAYER_TITLE_INTEGRITY 納入必須通過的正式 report。第 1～4 批稱號系統至此完成；Save Schema 維持 13，災厄、印記、戰鬥數值與故事資料均未更動。

- 2026-09-19 稱號上線後空白主畫面 hotfix：實際 GitHub Pages 測試出現外框正常但 #main 全空。逐檔以 JS parser 檢查第1～4批相關檔案後，確認 calamityui.js、savemigration.js、calamitycore.js 曾殘留由字串替換產生的字面 `\\n`，造成 `Invalid or unexpected token` 並中斷整體 script 載入鏈。已全部改為真正換行，並更新三檔 index cache-bust。隨後重新對 playertitlecore/playertitleintegrity/calamitygm/ui/calamityui/dungeonarena/dungeonbounty/dungeonvoidui/mirrordungeonui/mirrordungeonrun/specialencounter/runtimeintegrity/finalintegrity/savemigration/playernamerules/calamitycore/calamityrun 逐檔執行 JS parser，全部 parse OK。此 hotfix 不更動稱號規格、存檔語意、災厄數值或 Save Schema 13。

- 2026-09-19 本機存檔防覆寫保護 V1：因稱號上線期間 JS 語法錯誤曾造成主畫面空白，且使用者實測發現本地存檔被 Lv.1 新檔覆蓋，新增底層 SAVE_WRITE_GUARD_VERSION=1。engine.js 啟動時若偵測 SAVE_KEY 已有資料，正式 save() 在成功 load 前一律拒絕寫回 localStorage；savemigration.js 只有完整 parse + migrate + finalize 成功後才 markSaveLoadResolved("local-load") 並允許 autosave。若原本有本地檔但 JSON parse 失敗、root 非物件或 migration/finalize 丟錯，load() 明確回傳 false、保留原 localStorage、不解除寫入鎖，UI 只以暫時 state 顯示並跳過啟動 normalize/save。cloudsave.js 的明確下載流程在雲端資料 normalize 成功後可 markSaveLoadResolved("cloud-download")，因此仍能用雲端存檔救援。本機 guard 已加入 Runtime／Final Integrity；相關 JS parser 自檢均通過。Save Schema 仍為 13。
...
- 2026-09-19 鏡像戰稱號第1批完成：6 個稱號正式加入同一套 titles state，ID 依序 mirror_title_15～mirror_title_20，來源仍由 mirrorconfig 的正式 record title 資料延伸為 titleUnlocks。統一排序固定為 10 個災厄後接 15～20 勝鏡像稱號。normalizePlayerTitleState 會依 dungeon.mirror.history.bestWins 靜默補發舊存檔，且維持 additive，不產生 pending notice。正式鏡像 20 場結算時由 recordMirrorDungeonCompletion 呼叫 grantPlayerTitlesForMirrorWins；若 14→18，解鎖 15～18 並只回傳 noticeTitle=mirror_title_18；18→20 則解鎖 19、20 並只回傳 mirror_title_20。結算失敗 rollback 已包含 titles。這批尚未把鏡像稱號開放到角色欄 picker，也尚未做 6 種正式美術/GM 預覽。mirrorconfig 已提前於 playertitlecore 載入。4 個改動 JS 重新抓 main 後 new Function 語法解析全通過，並以實際函式 probe 驗證 14→18、18→20 與 bestWins=17 靜默 backfill 均通過。

- 2026-09-19 鏡像戰稱號第2批完成：鏡像稱號正式開放至同一個角色稱號 picker，`getUnlockedPlayerTitleDefinitions` 改依統一 `ALL_DEFS` 順序回傳，因此角色欄固定災厄在前、鏡像在後，只顯示已解鎖項目。`playerTitleHtml` 改為 series-aware：災厄輸出 `player-title--calamity player-title--tier-N`，鏡像輸出 `player-title--mirror player-title--mirror-WINS`，不再產生 tier-undefined。鏡像稱號可由同一個 `equipPlayerTitle` 裝備，並透過既有 `playerIdentityNameHtml` 出現在玩家名稱前；劇情 `storyui.js` 與戰線紀錄 `storyrecordtabs.js` 仍不呼叫稱號 renderer，維持純玩家名稱。第3批正式六種美術前，playertitles.css 先提供中性銀白/淡紫可見基準，避免角色欄或名稱顯示透明。JS 重新抓 main 後 new Function 全通過，實際 probe 驗證災厄2個+鏡像15～18排序、mirror_title_18 裝備、名稱前 renderer、無 tier-undefined、劇情/戰線排除均通過。cache-bust：playertitlecore.js?v=20260919-mirror-title-batch2、playertitles.css?v=20260919-mirror-title-batch2。

- 2026-09-19 鏡像戰稱號第3批完成：6 個鏡像稱號正式美術完成，維持同一 player-title renderer 與開放式 aura，不做封閉框。15「幸運眷顧」＝薄荷青/銀白/淡金的概率光點與不對稱漂移；16「天選之刻」＝冰白/天藍/淡金的時間切片與瞬時閃光；17「逆命者」＝深紅/冷白/紫黑的因果裂痕與錯位 glitch；18「傳說之日」＝暖金/晨白的史詩晨曦與水平擴光；19「距神一步」＝象牙白/聖金/淡虹的神性門檻、上升光層與 halo；20「神蹟」＝純白/虹彩/淡金紫藍的現實失常、折射與空間分裂。每個稱號都有獨立 main/::before/::after 與 3 組專屬 keyframes，非換色。六個均無 border/clip-path 封閉框，手機/桌機共用，prefers-reduced-motion 可停動畫但保留靜態視覺。cache-bust：playertitles.css?v=20260919-mirror-title-batch3。自檢確認 CSS 結構平衡、六套動畫完整、無符號框。

- 2026-09-19 鏡像戰稱號第4批完成：鏡像正式取得通知已接回既有共用 pendingNotice／civilizationTitleNoticeModal owner；新鏡像紀錄跨多階時僅最高新稱號寫入 pendingNotice，舊檔 normalize/backfill 仍靜默。鏡像 20 場正式結算 render 後呼叫既有 showPendingPlayerTitleNotice；通知文案依 def.series 區分災厄首殺與鏡像歷史最高勝場。GM 稱號預覽升級 V2，改讀統一 PLAYER_TITLE_DEFS，固定前10災厄、後6鏡像，並在第11項前加入鏡像分隔；仍純 runtime、不得修改 state/localStorage。PLAYER_TITLE_INTEGRITY 升 V2，新增 10+6 定義/排序、鏡像 silent backfill、14→18 多稱號解鎖且只通知最高、鏡像 renderer、GM V2 無副作用檢查。runtime/final integrity 已鎖定 PLAYER_TITLE_STATE V1 + Integrity V2 + GM Preview V2 + 災厄10/鏡像6/總16。所有7個改動 JS 重新抓 main 後 new Function 語法解析全通過；實際 core probe 驗證 bestWins=17 舊檔補發 pending=null、14→18 pending=mirror_title_18。cache-bust 已更新至 mirror-title-batch4。

- 2026-09-19 鏡像稱號高階化第1批完成：只強化 mirror 15～18，不動 19、20。15「幸運眷顧」擴大機率場、增加更多偏移光點/雙側概率構造；16「天選之刻」增加多層時間切片與停格閃光；17「逆命者」擴大因果裂隙、增加多重錯位/斷裂層與更強 glitch；18「傳說之日」擴大晨曦場、加入更廣的水平史詩光域與雙側升起構造。15～18 字體略放大為 1.09/1.10/1.11/1.12em，外擴範圍與動畫振幅全面提高；17/18 的 before 水平外擴已達/超過災厄10的 -1.12em。仍無封閉 border/clip-path，手機/桌機共用、reduced-motion 保留。19/20 原樣未動。cache-bust：playertitles.css?v=20260919-mirror-high-batch1。自檢全通過。

- 2026-09-19 鏡像稱號15～18再次高階化：本次不是微調，直接把外擴、層次、字級與事件動畫拉到明顯高於災厄8～10。字級改為15=1.13em、16=1.14em、17=1.16em、18=1.18em；災厄8～10仍為1.08em。before水平外擴分別達-1.62/-1.72/-1.88/-2.00em，災厄10為-1.12em；after也同步擴大。15增加大量概率光點/概率翼與更強跳位，16增加多層時間切片與多段停格閃光，17增加更大因果裂隙、雙向錯位與強glitch，18增加超寬晨曦/史詩光域與更大升起構造。仍無封閉border/clip-path，19/20完全未動，reduced-motion保留。cache-bust：playertitles.css?v=20260919-mirror-high-batch1b。自檢確認CSS結構、外擴、字級、19/20未動與cache均通過。

- 2026-09-19 GM 稱號預覽改為實戰名稱預覽：移除原本純稱號展示，GM Preview 升 V3。現在下拉仍固定 16 個（前10災厄、後6鏡像），預覽區直接使用目前正式 playerName 與正式 `playerIdentityNameHtml({name,titleId,compact:true})`，置於實際 `.combatant.player > h2` 結構中，並附 HP 100/100 條作為戰鬥畫面尺寸參考。GM 預覽不呼叫 equipPlayerTitle、不 save、不寫 state.titles，只讀玩家名稱與正式 renderer。`PLAYER_TITLE_INTEGRITY_VERSION` 升 V3，runtime/final 同步鎖定 GM Preview V3 + Integrity V3。playertitles.css 新增 GM 戰鬥名稱預覽容器尺寸，手機仍沿用正式 combat identity wrap。所有4個改動 JS 重新抓 main 後 `new Function` 解析通過；預覽 owner scope 無 equip/save/title state write；cache-bust 已更新為 `title-combat-preview1`。

- 2026-09-19 GM 實戰名稱預覽玩家名不可見修正：正式 `.player-identity-name` 不再只依賴 `color:inherit`，明確鎖定 `-webkit-text-fill-color:currentColor`、背景裁切還原、opacity/visibility，避免同一 identity 內稱號的透明文字填色效果污染普通玩家名。GM 戰鬥預覽另外對 `.gm-player-title-combat-preview .player-identity-name` 加金色可見 fallback（#ebcb84），但稱號本體仍保留 transparent + gradient。cache-bust 更新 `playertitles.css?v=20260919-title-name-visible1`。自檢確認 CSS 結構、正式 renderer、正式玩家名來源、稱號透明漸層與 cache 全通過。

- 2026-09-19 鏡像稱號高階化第2批完成：專門強化 mirror 19「距神一步」與20「神蹟」，確保位階明顯高於15～18。19 字級1.22em、before外擴-1.00/-2.25em、after-0.76/-1.70em，加入更大的不完整神性光環、上升光層、雙側聖性弧與垂直階梯式光帶；20 字級1.28em、before外擴-1.12/-2.65em、after-0.88/-2.00em，加入最多層的虹彩折射、星點、雙側現實裂解、中央重組場與多段空間切片，動畫速度加快至文字1.18s、warp1.08s、split0.92s，作為目前全遊戲最浮誇稱號。18→19→20 字級、外擴、layer count均逐級提高；layer count 18=12、19=13、20=20。GM 實戰名稱預覽與玩家名可見修正保留，無封閉border/clip-path，reduced-motion正常。cache-bust：playertitles.css?v=20260919-mirror-high-batch2。自檢全通過。

- 2026-09-19 稱號系統優化第1批完成（舊檔處理未動）：新增 `playertitleui.js` 作為唯一共用稱號 UI owner，正式接管 picker、pending notice modal、visibility replay、select/save/render；`playertitlecore.js` 移除 UI/save/render owner，只保留定義/state/解鎖/renderer/equip mutation。`calamityui.js` 已移除稱號通知 modal owner，改呼叫 `queuePendingPlayerTitleNotice()`；鏡像戰仍使用同一共用 API。裝備稱號與清除 pending notice 現在都先保留舊值，`save(false)===false` 時 rollback 並不關閉 UI。正式 `playerIdentityNameHtml()` 對明確 `titleId` 預設要求 target 已擁有該稱號；只有 `allowUnownedTitle:true` 才可繞過，GM 實戰預覽已明確使用此 preview bypass。`PLAYER_TITLE_UI_VERSION=1`、`PLAYER_TITLE_INTEGRITY_VERSION=4`，runtime/final 同步鎖定。index 載入順序為 core→UI→後續使用者。7 支改動 JS 全部重新抓 main 後 `new Function` 解析通過；功能 probe 確認未擁有稱號正式 renderer 會阻擋、GM bypass 可顯示、裝備 save 失敗可 rollback、pending notice save 失敗可 rollback。cache-bust：`20260919-title-opt-batch1`。

- 2026-09-19 稱號系統優化第2批完成（舊檔處理未動）：`calamityconfig.js` 升 V2 並成為災厄稱號 metadata 正式 owner，每列直接持有 `titleId/titleName`；鏡像稱號 metadata 繼續由 `mirrorconfig.js.titleUnlocks` 持有。`playertitlecore.js` 已移除災厄稱號名稱硬編碼與 renderer，現在只讀兩個 config、組合定義並負責 state／normalization／解鎖／equip mutation。新增 `playertitlerenderer.js`（`PLAYER_TITLE_RENDERER_VERSION=1`）作為唯一 `playerTitleHtml/playerIdentityNameHtml` owner；index 載入順序固定為 calamity/mirror config → title core → renderer → UI。`PLAYER_TITLE_INTEGRITY_VERSION` 升 V5，改以 config metadata 驗證定義，不再另外硬編碼 16 個名稱；renderer/UI/GM 詳細版本檢查集中於 title integrity。`runtimeintegrity.js` 與 `finalintegrity.js` 已移除對 `PLAYER_TITLE_UI_VERSION`／`GM_PLAYER_TITLE_PREVIEW_VERSION`／`PLAYER_TITLE_INTEGRITY_VERSION` 精確值的直接硬鎖，改信任 `PLAYER_TITLE_INTEGRITY.passed` 並只保留 10/6/16 公開定義數量檢查；Calamity Config V2 鏈同步更新 runtime/final/calamitystateintegrity。7 支改動 JS 全部重新抓 main 後 `new Function` 解析通過；功能 probe 確認災厄/鏡像 config metadata 正確、normalization 解鎖語意未變、正式 renderer 仍阻擋未取得稱號、GM preview bypass 可用。cache-bust：`20260919-title-opt-batch2`。

- 2026-09-19 稱號系統優化第3批完成：保留鏡像敵人完整複製玩家稱號的現有設計，不改遊戲語意；新增 `MIRROR_TITLE_CLONE_PERFORMANCE_VERSION=1`，鏡像敵方戰鬥卡加上 `.mirror-title-clone-host`。桌機仍維持玩家與鏡像敵人兩套完整稱號動畫；手機 `max-width:760px` 僅停止鏡像敵人複製稱號的 `.player-title/::before/::after` 動畫，靜態文字、漸層、外擴與稱號本身全部保留，玩家自己的稱號動畫完全不受影響，等同把手機鏡像戰最重的重複稱號動畫負擔減半。`PLAYER_TITLE_INTEGRITY_VERSION` 升 V6，由 title integrity 檢查 mirror clone performance contract。`prefers-reduced-motion` 全域規則仍保留。`mirrordungeonrun.js` 與 `playertitleintegrity.js` 回讀 main 後 `new Function` 解析通過；CSS 自檢確認 19/20 原動畫完整、桌機未降級、手機只停 enemy clone、cache-bust 正確。cache：`20260919-title-opt-batch3`。
