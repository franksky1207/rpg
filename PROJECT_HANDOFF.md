...
>
> 本文件於 **2026-09-11** 重新整理，已納入：完整專精、VIP、副本、特殊怪、遊戲說明頁、玩家端取消戰鬥紀錄、競技場自動三連戰、主線勝利後特殊遭遇、連戰失敗保留收益顯示，以及存檔安全強化。
...
- `ui.js?v=20260911-1555-savesafety`
...
主線連戰若中途死亡：
- 前面已勝利場次的 EXP、金幣、裝備與副本進度都保留。
- 結算頁會先顯示已保留的前段收益，再顯示本次死亡懲罰。
...
- 「放棄」遺失裝備屬永久刪除，現在會先顯示確認對話框，確認後才刪除。
...
啟動時在既有 `load()` 完成後再做一次核心 state 安全整理並存回。
...
不合法裝備資料會被移除或修成安全值；HP 最後會再透過 `normalizeHP()` 限制在目前最大 HP 範圍內。

### 18.2 匯入存檔

匯入不再只檢查 `level` truthy：
- JSON 必須是物件。
- `level` 必須為有效正數。
- 必須至少帶有正式存檔識別結構之一（例如 `saveVersion`、`equipment`、`inventory`、`mapProgress`）。
- 通過格式檢查後，還會先跳出「將覆蓋目前遊戲進度」確認。
- 使用者取消時不會覆蓋目前存檔。
- 確認後先 normalize，再存入並重新載入。
...
- `gameguide.js`：玩家說明內容
- `guide.css`：玩家說明版型
- `settlementui.js`：主線結算補充、連戰失敗保留收益顯示、VIP 結算附加
...
- 特殊怪只在主線勝利後檢查；第一場開打前不觸發。
- 單場與連戰最後一場勝利都可正常觸發特殊怪。
- 連戰中途死亡時，前段已取得收益會在死亡懲罰前顯示。
- 先制 Lv0 不顯示 `先制！`。
- 反擊不遞迴，但可再連擊。
...
### 存檔／商店
- 正常現有存檔載入後資料不變形。
- 舊存檔缺欄位時可由 normalize 補成安全值。
- 非存檔 JSON 或無效 level 會被拒絕。
- 合法匯入檔在覆蓋目前進度前會先要求確認。
- 取消匯入不覆蓋目前資料。
- 異常負值 gold／exp／hp 會被整理成合法非負值。
- 永久放棄遺失裝備前會要求確認。
...
>
> 截至 2026-09-11：主線 Lv1～100、VIP0～20、專精8項 Lv0～30、三大副本、特殊怪、GM 五種測試與遊戲說明均已正式整合。玩家端戰鬥紀錄已取消；競技場現在按一次「開始三戰」後自動連打到結束；特殊怪只在主線勝利後檢查；存檔匯入已有格式驗證、normalize 與覆蓋確認。最新平衡原則仍是「主線刷成長與資源、特殊怪當驚喜、副本負責挑戰」。