...
- `dungeonvoid.js` 在 `traits.js` 後載入，第一批只提供虛空幻境資料、公式、名稱、特性與存檔欄位，不接正式UI。
...
正式名稱：**虛空幻境**；Lv25解鎖；定位為無限樓層爬塔副本。第一批已新增 `dungeonvoid.js`，尚未接入正式戰鬥與UI。

核心原則：
- 無限樓層，敵人**完全不參考玩家能力或玩家等級**，只由樓層決定固定基礎能力。
- 塔內部戰力座標：`E = 24 + floor / 10`。
- 基礎能力：
```js
...
```
- 所有樓層基礎暴擊固定10%、基礎閃避固定8%，不隨樓層成長；迅捷／致命／巨體等特性仍可依既有特性效果改變當次實際值。
- 普通樓層固定1個隨機特性；每10層固定2個不同特性；重新挑戰時重新隨機。
- 每10層不額外增加HP／ATK／DEF，雙特性即為該關特殊強度。
- 預定正式戰鬥為每層開始前完全回血，1次挑戰次數一路自動往上打，直到戰敗或玩家要求本層結束後強制退出；第二批才接戰鬥流程。

普通怪名稱池10個，隨機抽取且不可與上一個普通怪名稱重複：
- 虛影獵手／幻境遊魂／裂隙行者／異相戰影／迷界殘像
- 虛空獵兵／幻域追獵者／失真戰體／空間殘響／幻象執行者

每10層王關名稱依序輪替，100層一循環：
1. 10層 幻境獵王
2. 20層 虛影統領
3. 30層 裂隙霸主
4. 40層 異相主宰
...
9. 90層 虛境帝皇
10. 100層 虛無終焉者
110層重新從「幻境獵王」開始，但實際能力仍依樓層公式持續提高。
...
存檔欄位：`state.dungeon.voidMirage.highestCleared`，舊存檔會自動補為0；`getVoidMirageNextFloor()` 為最高通過+1。第一批已有連續進度記錄函式，但真正首通發分／戰鬥結算於第二批接入。

後續預定：
- 第二批：正式自動連戰、每層滿血、首通進度與積分、失敗、強制退出。
- 第三批：正式副本UI。
- 第四批：GM指定起始樓層連續爬塔測試，顯示成功層數、總／平均積分等，再校正 `2.40 / 2.15 / 2.65`。
...
- `dungeonvoid.js`：虛空幻境無限樓層核心公式／固定暴擊閃避／特性／命名／首通積分／永久樓層存檔
