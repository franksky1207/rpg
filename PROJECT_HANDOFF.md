...
1. 懸賞戰（Lv5）
2. 競技場（Lv15）
3. **虛空幻境（Lv25，核心＋正式戰鬥流程已建立，UI待第三批）**
...
...
<script src="dungeonbounty.js"></script>
<script src="dungeonarena.js"></script>
<script src="dungeonvoid.js"></script>
...
...
- `dungeonvoid.js` 在 `traits.js` 後載入，現在已包含虛空幻境資料、公式、永久進度、正式進場、自動連戰、首通發分、失敗與強制退出核心；正式畫面由第三批接入。
...
正式名稱：**虛空幻境**；Lv25解鎖；無限樓層。`dungeonvoid.js` 已完成核心資料與正式戰鬥流程，正式UI待第三批。

核心原則：
- 敵人完全不參考玩家能力或玩家等級，只由樓層決定。
- `E = 24 + floor / 10`。
- 基礎能力：
```js
...
```
- 基礎暴擊10%、基礎閃避8%，不隨樓層成長；特性仍可改變實際值。
- 普通層1個隨機特性；每10層2個不同特性；失敗後下次重打會重新抽。
- 每10層不額外乘能力倍率，雙特性本身就是特殊關強度。

正式戰鬥流程：
- `beginVoidMirageRun()`：Lv25以上且有副本可挑戰次數時，消耗1次並從 `highestCleared + 1` 開始。
- **1次副本可挑戰次數 = 1次完整虛空幻境挑戰。**
- 每一層開始前玩家HP完整恢復。
- `fightNextVoidMirageFloor()`：打當前樓層；使用共通 `dungeonFightCore()` 與200回合保護。
- 勝利：永久記錄樓層、立即發放該層首通積分、完整回血、準備下一層。
- 戰敗或200回合未分勝負：本次挑戰結束；該層不記通過、0積分；結束後完整回血。
- 不給主線EXP／金幣／裝備，不觸發主線死亡懲罰，也不產生副本進度。
- `runVoidMirageAuto()`：會自動一層一層往上執行直到失敗／timeout／強制退出；每層後都讓出事件迴圈，第三批UI可在自動連戰期間接受退出操作。
- `requestVoidMirageExit()`：強制退出不退還已消耗的挑戰次數；如果當層正在處理，該層結束後退出；若位於兩層之間則直接結束，不會再進下一層。
- 已經成功取得的樓層進度與首通積分全部保留。

普通怪名稱池10個，隨機且不可與上一個普通怪同名：
...
永久存檔欄位：`state.dungeon.voidMirage.highestCleared`。
本次挑戰的起始樓層、當前樓層、成功層數、本次積分、總回合、平均回合、最後敵人、失敗樓層、退出狀態等保留在 `voidMirageRun` 執行期快照，供第三批UI與第四批GM測試使用。

後續：
- 第三批：正式副本卡／進塔／戰鬥／本次統計／強制退出UI，桌機與手機一起整理。
- 第四批：GM指定起始樓層連續爬塔測試，顯示成功層數、總積分、平均積分、平均回合、最後成功層剩餘HP等，再校正 `2.40 / 2.15 / 2.65`。
...
- `dungeonvoid.js`：虛空幻境無限樓層公式／特性／命名／永久進度／正式進場／自動連戰／首通積分／失敗與強制退出核心
