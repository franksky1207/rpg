// 永久回歸檢查：銀河 101 篇正式劇情＋宇宙 10 區／100 Boss 劇情架構。
const fs=require('fs');
const vm=require('vm');
...
try{
 ...
 const report=context.STORY_INTEGRITY_REPORT;if(!report){console.error('STORY_INTEGRITY_REPORT 不存在');process.exit(2);}if(Number(report.version)<11){console.error(`STORY_INTEGRITY_VERSION 過舊：${report.version}`);process.exit(4);}
 const expectedPolicy={intro:{pages:12,minChars:90,maxChars:155,minBlocks:3},regularBoss:{pages:11,minChars:90,maxChars:120,minBlocks:2},regionFinale:{pages:15,minChars:120,maxChars:155,minBlocks:3},finalBoss:{pages:31,minChars:90,maxChars:155,minBlocks:3}};
 if(JSON.stringify(report.formatPolicy||{})!==JSON.stringify(expectedPolicy)){console.error(`STORY FORMAT POLICY 不一致：${JSON.stringify(report.formatPolicy)}`);process.exit(6);}
 ...
 if(Number(report.totalStoriesTarget)!==201){console.error(`最終劇情目標應為 201，實際 ${report.totalStoriesTarget}`);process.exit(9);}
 if(report.passed){console.log(`STORY INTEGRITY PASSED | galaxy=${report.galaxyStories}/101 universe=${report.universeStoriesLoaded}/100 registry=${report.universeStoriesExpected}/100 target=${report.totalStoriesTarget}`);process.exit(0);}
