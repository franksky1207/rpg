// 永久回歸檢查：銀河與宇宙正式 storydata 顯示文字必須本身為中文；不得依賴 Story Integrity 先行修正。
const fs=require('fs');
const vm=require('vm');
...
for(const file of files){
 if(!fs.existsSync(file))throw new Error(`缺少正式劇情來源檔：${file}`);
...
function checkNarrative(value,where){const text=String(value??'');const found=bannedNarrativeTerms.filter(term=>text.includes(term));if(found.length)failures.push({where,type:'internal-term',terms:found,text});}
function checkRegions(regions,era){
 for(const region of regions||[]){
  check(region?.name,`${era} ${region?.id||'unknown'} region.name`);
  for(const row of Array.isArray(region?.stories)?region.stories:[])check(row?.label,`${era} ${row?.id||'unknown'} registry.label`);
 }
}
checkRegions(context.CIVILIZATION_STORY_REGIONS,'galaxy');
checkRegions(context.CIVILIZATION_UNIVERSE_STORY_REGIONS,'universe');
for(const [id,story] of Object.entries(context.CIVILIZATION_STORIES||{})){
 check(story?.chapter,`${id} chapter`);check(story?.location,`${id} location`);check(story?.title,`${id} title`);
...
console.log(`STORY SOURCE PURITY PASSED: galaxyRegions=${(context.CIVILIZATION_STORY_REGIONS||[]).length} universeRegions=${(context.CIVILIZATION_UNIVERSE_STORY_REGIONS||[]).length} loadedStories=${Object.keys(context.CIVILIZATION_STORIES||{}).length}`);
