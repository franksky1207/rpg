(function(){
 ...
 if(actualStoryIds.length!==101)fail("STORY_TOTAL_COUNT",`正式故事總數應為 101（序章＋100 Boss），實際 ${actualStoryIds.length}`);
 ...
 if(bossStoryCount!==100)fail("STORY_BOSS_CHECK_COUNT",`應檢查 100 個 Boss 故事，實際 ${bossStoryCount}`);
 ...
 window.STORY_INTEGRITY_VERSION=3;
 window.STORY_INTEGRITY_REPORT=report;
 window.runCivilizationStoryIntegrity=()=>report;
