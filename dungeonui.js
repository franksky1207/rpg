(function(){
 ...
 function injectDungeonStyles(){
  ...
  .dungeon-mode-mirror{background:linear-gradient(180deg,#23232d,#171720);border-color:#8b84aa;box-shadow:0 12px 30px rgba(26,23,40,.28)}.dungeon-mode-mirror h3{color:#e4e2ee}.dungeon-mode-mirror .dungeon-mode-reward{color:#c9b8ec}.dungeon-mode-mirror .dungeon-entry-btn{background:#58516f;border-color:#9085b0;color:#fff}.dungeon-mode-mirror .dungeon-entry-btn:not(:disabled):hover{background:#686080}.mirror-history-summary{margin:12px 0 0;color:#d5d2df;font-size:13px;font-weight:700;line-height:1.55}
 ...
 function mirrorHomeCardHtml(){
  const config=window.MIRROR_DUNGEON_CONFIG;if(!config||typeof mirrorDungeonStatus!=="function")return "";
  const info=mirrorDungeonStatus(),unlock=Math.max(1,Number(config.unlockLevel)||50),battles=Math.max(1,Number(config.runBattles)||20),unlocked=!!info.unlocked,ended=!!info.ended;
  const history=info.history||{},hasRecord=!!history.bestDate,best=typeof window.mirrorDungeonClampWins==="function"?window.mirrorDungeonClampWins(history.bestWins):Math.max(0,Math.floor(Number(history.bestWins)||0)),miracles=Array.isArray(history.miracleDates)?history.miracleDates:[],title=hasRecord&&typeof window.mirrorDungeonRecordTitle==="function"?window.mirrorDungeonRecordTitle(best):"";
  const historyHtml=`<div class="mirror-history-summary">${hasRecord?`歷史最高：${best} 勝${title?`　・　${title}`:""}`:"歷史最高：尚無紀錄"}${miracles.length?`<br>神蹟：${miracles.length} 次`:""}</div>`;
  const statusText=unlocked?(ended?"今日鏡像戰已結束":info.status==="running"?"今日鏡像戰進行中":"今日尚未挑戰"):`Lv.${unlock} 解鎖`,buttonLabel=!unlocked?"尚未解鎖":ended?"查看鏡像戰":"進入鏡像戰";
  return `<section class="dungeon-mode-card dungeon-mode-mirror${unlocked?"":" locked"}" data-mirror-dungeon-card="1"><div class="dungeon-mode-head"><div><h3>鏡像戰</h3><div class="dungeon-mode-reward">VIP 積分</div></div><span class="dungeon-unlock-label">${unlocked?`Lv.${unlock} 已解鎖`:`Lv.${unlock} 解鎖`}</span></div>${historyHtml}<p>與完全相同的自己連續戰鬥 ${battles} 場，勝場越高，獎勵越豐厚。</p><div class="dungeon-cost">${statusText}</div><button class="btn dungeon-entry-btn" ${unlocked?"":"disabled"} onclick="${unlocked?"openMirrorDungeon()":"void(0)"}">${buttonLabel}</button></section>`;
 }
