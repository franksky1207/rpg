(function(){
 ...
 function dungeonHomeHtml(){
  ...
  return `<div class="function-page dungeon-page-shell"><div class="back-home"><button class="btn back-btn" onclick="go('home')">← 返回主頁</button></div><section class="card dungeon-summary-panel"><h2>副本</h2><div class="dungeon-summary-grid"><div><span class="muted">可挑戰次數</span><strong>${d.attempts} 次</strong></div><div><span class="muted">VIP 狀態</span><strong>${typeof vipStatusText==="function"?vipStatusText():`VIP${state.vipLevel||0}`}</strong></div></div></section><div class="dungeon-mode-list">${card("bounty","懸賞戰","積分・金幣・裝備","隨機挑戰一名依你目前實力生成的強敵。",DUNGEON_UNLOCKS.bounty)}${card("arena","競技場","積分","連續挑戰三名敵人，考驗整體續戰能力。",DUNGEON_UNLOCKS.arena)}${card("tower","虛空幻境","積分","逐層挑戰越來越強的敵人。",DUNGEON_UNLOCKS.tower)}</div></div>`;
 }
 ...
 function ensureHomeDungeonCard(main){
  ...
  const adventure=menu.querySelector(".menu-card"),wrap=document.createElement("div");wrap.innerHTML=`<button class="menu-card" data-dungeon-home-card="1" onclick="go('dungeon')"><b>副本</b><span>挑戰副本取得積分與各類獎勵</span></button>`;
