(function(){
 ...
 if(!Number.isInteger(BOSS_MAX_HP)||BOSS_MAX_HP<=0)throw new Error("Third-world data requires THIRD_WORLD_BOSS_MAX_HP owner.");
 ...
 const BOSS_ROWS=Object.freeze([
  {persistedIndex:0,id:"higher-dimensional-boss-01",name:"破界天裁",specialization:Object.freeze({id:"attack",atkMultiplier:1.15})},
  {persistedIndex:1,id:"higher-dimensional-boss-02",name:"永劫重垣",specialization:Object.freeze({id:"defense",defMultiplier:1.15})},
  {persistedIndex:2,id:"higher-dimensional-boss-03",name:"宿因天秤",specialization:Object.freeze({id:"critical",critPoints:6})},
  {persistedIndex:3,id:"higher-dimensional-boss-04",name:"無相彼岸",specialization:Object.freeze({id:"dodge",dodgePoints:6})},
  {persistedIndex:4,id:"higher-dimensional-boss-05",name:"萬象迴演",specialization:Object.freeze({id:"combo",comboRatePoints:10})},
  {persistedIndex:5,id:"higher-dimensional-boss-06",name:"維隙之刃",specialization:Object.freeze({id:"penetration",penetrationRatePoints:10})},
  {persistedIndex:6,id:"higher-dimensional-boss-07",name:"逆因輪轉",specialization:Object.freeze({id:"counter",counterRatePoints:10})},
  {persistedIndex:7,id:"higher-dimensional-boss-08",name:"噬界深淵",specialization:Object.freeze({id:"drain",drainRatePoints:10})},
  {persistedIndex:8,id:"higher-dimensional-boss-09",name:"先驗之瞳",specialization:Object.freeze({id:"initiative",initiativeBonusPoints:20})},
 ...
 ].map(row=>Object.freeze({...row,thresholdRemainingHp:Math.round(BOSS_MAX_HP*row.thresholdRemainingPercentSum/100),series:"higher-dimensional"})));
 ...
 function thirdWorldBossStage(currentHp,maxHp=BOSS_MAX_HP){
  ...
  let stage=0;
  for(let threshold=90;threshold>=10;threshold-=10){if(hp*100<=max*threshold)stage++;else break;}
 ...
 function thirdWorldBossStats(value,currentHp=BOSS_MAX_HP){
  ...
  const stats={index:boss.index,id:boss.id,name:boss.name,maxHp:boss.maxHp,stage,atk:BASE_STATS.atk+stage*STAGE_CONFIG.atkPerStage,def:BASE_STATS.def+stage*STAGE_CONFIG.defPerStage,crit:BASE_STATS.crit+stage*STAGE_CONFIG.critPointsPerStage,dodge:BASE_STATS.dodge+stage*STAGE_CONFIG.dodgePointsPerStage,initiativeBonusPercent:BASE_STATS.initiativeBonusPercent,comboRate:BASE_STATS.comboRate,penetrationRate:BASE_STATS.penetrationRate,counterRate:BASE_STATS.counterRate,drainRate:BASE_STATS.drainRate};
 ...
 function bossStoredHp(index,target=null){const boss=BOSS_ROWS[index];if(!boss)return 0;const s=targetState(target),raw=s?.thirdWorld?.bosses?.[index]?.currentHp;return clamp(finiteWhole(raw,boss.maxHp),0,boss.maxHp);}
 function bossProgressCore(index,target=null){
  const boss=BOSS_ROWS[index];if(!boss)return null;
  const currentHp=bossStoredHp(index,target),maxHp=boss.maxHp,remainingRatio=currentHp/maxHp,remainingPercent=remainingRatio*100,remainingBasisPoints=Math.round(remainingRatio*10000),defeated=currentHp<=0;
  return Object.freeze({index:boss.index,id:boss.id,name:boss.name,maxHp,currentHp,remainingRatio,remainingPercent,remainingBasisPoints,alive:!defeated,defeated,stage:thirdWorldBossStage(currentHp,maxHp),stats:thirdWorldBossStats(index,currentHp),abilities:thirdWorldBossAbilities(index,currentHp)});
 }
 ...
 function thirdWorldBossProgressSnapshot(value,target=null){const index=bossIndex(value);if(index<0)return null;const core=bossProgressCore(index,target),challenge=thirdWorldChallengeStatus(index,target);return Object.freeze({...core,challengeAllowed:challenge.allowed===true,challengeStatus:challenge});}
 ...
 function thirdWorldTitleTierForRemainingHp(value){
  const hp=Number(value);
  if(!Number.isInteger(hp)||hp<0||hp>TOTAL_BOSS_MAX_HP)return 0;
  if(hp===0)return 10;
  for(let index=8;index>=0;index--){const def=TITLE_ROWS[index];if(hp<=def.thresholdRemainingHp)return def.tier;}
  return 0;
 }
 function thirdWorldTitleTierForRemainingPercentSum(value){
  const total=Number(value);
  if(!Number.isFinite(total)||total<0||total>BOSS_COUNT*100)return 0;
 ...
 window.THIRD_WORLD_BOSS_PERSISTENCE_ORDER=Object.freeze(BOSS_ROWS.map(row=>row.id));
 ...
 window.thirdWorldTitleTierForRemainingHp=thirdWorldTitleTierForRemainingHp;
