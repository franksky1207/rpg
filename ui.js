...
function adventurePage(){
 ...
 let battleControls=e.kind==="boss"?`<div class="controls"><button class="btn primary" onclick="startBattles()">挑戰 Boss</button><button class="btn ok" onclick="rest()">回城休息</button></div><input type="hidden" id="battleCount" value="1">`:`<h3>連續戰鬥</h3><div class="seg" id="battleSeg">${opts}</div><input type="hidden" id="battleCount" value="${selectedBattleCount}"><div class="controls"><button class="btn primary" onclick="startBattles()">開始戰鬥</button><button class="btn ok" onclick="rest()">回城休息</button></div>`;
...
function sellLowerAll(){
 let targets=state.inventory.filter(it=>{
   if(it.locked||it.q===5)return false;
   let current=state.equipment[it.type];
