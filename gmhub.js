(function(){
 ...
 function installGmHubStyles(){
  ...
  style.textContent=`
   ...
   .gm-hub-section>summary::-webkit-details-marker{display:none}
   .gm-hub-section>summary::after{content:"＋";float:right;color:#a99b7f}
   .gm-hub-section[open]>summary::after{content:"－"}
 ...
 function specialOptions(){
  ...
  return SPECIAL_MONSTERS.map(x=>`<option value="${x.id}">${x.name}（${tierLabel[x.tier]||"低"}）</option>`).join("");
 }
 function gearOptions(){
  return {
   quality:QUALITY.map((q,i)=>`<option value="${i}" ${i===3?"selected":""}>${q.n}</option>`).join(""),
   level:Array.from({length:50},(_,i)=>{const lv=i+1;return `<option value="${lv}" ${lv===state.level?"selected":""}>Lv.${lv}</option>`}).join(""),
   type:EQUIPMENT_TYPES.map(type=>`<option value="${type}">${equipmentTypeLabel(type)}</option>`).join("")
  };
 }
 function dungeonMapOptions(){
  return MAPS.map((map,i)=>`<option value="${i}">${i+1}. ${map.name}（Lv.${map.min}～${map.max}）</option>`).join("");
 }
 function dungeonEnemyOptions(mapIdx=0){
  const i=Math.max(0,Math.min(MAPS.length-1,Number(mapIdx)||0));
  return MAPS[i].enemies.map((e,j)=>`<option value="${j}">${e[2]==="boss"?"Boss":e[2]==="elite"?"菁英":"普通"}｜${e[0]} Lv.${e[1]}</option>`).join("");
 }
 ...
 function bountyTestHtml(){
  const result=typeof getBountyGmDebugHtml==="function"?getBountyGmDebugHtml():"";
