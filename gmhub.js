(function(){
 ...
 function generalManagementHtml(){
  ...
  return `<div class="notice gm-hub-note">目前世界：${worldLabel}　／　角色有效等級上限 Lv.${cap}</div><div class="controls"><button class="btn" onclick="gmLevel()">指定等級</button><button class="btn" onclick="gmGold()">指定金幣</button><button class="btn" onclick="gmSetWorldProgress()">指定銀河紀元解鎖進度</button><button class="btn danger" onclick="gmResetVip()">重置 VIP（等級＋積分）</button></div>
  <div class="item" style="margin-top:12px"><b>產生裝備</b><div class="muted" style="margin-top:5px">目前仍為銀河紀元裝備產生器；世界 2 裝備將於裝備批次接入。</div><div class="controls" style="align-items:end"><label>品質<br><select id="gmGearQuality" class="btn">${g.quality}</select></label><label>等級<br><input id="gmGearLevel" class="btn" type="number" inputmode="numeric" min="1" max="${MAX_LEVEL}" step="1" value="${Math.max(1,Math.min(MAX_LEVEL,Math.floor(Number(state.level)||1)))}"></label><label>部位<br><select id="gmGearType" class="btn">${g.type}</select></label><button class="btn gm-create" onclick="gmCreateGear()">產生裝備</button></div></div>`;
 }
 ...
 function mapMonsterTestHtml(){const s=mapMonsterSelection(),result=typeof getMapMonsterGmTestHtml==="function"?getMapMonsterGmTestHtml():"",secondOptions=typeof getSecondWorldBossGmOptions==="function"?getSecondWorldBossGmOptions():"",secondResult=typeof getSecondWorldBossGmTestHtml==="function"?getSecondWorldBossGmTestHtml():"";return `<div class="muted gm-hub-note">快速單怪功能測試：銀河紀元維持原地圖怪；宇宙紀元可直接指定 100 隻主線 Boss。兩者皆為沙盒模式，不修改正式角色資料。</div><div class="item"><b>銀河紀元</b><div class="controls" style="align-items:end;margin-top:8px"><label>區域<br><select id="gmMapMonsterRegion" class="btn" onchange="gmMapMonsterChangeRegion()">${mapMonsterRegionOptions()}</select></label><label>地圖<br><select id="gmMapMonsterMap" class="btn" onchange="gmMapMonsterChangeMap()">${mapMonsterMapOptions(s.regionIdx)}</select></label><label>怪物<br><select id="gmMapMonsterEnemy" class="btn" onchange="gmMapMonsterChangeEnemy()">${mapMonsterEnemyOptions(s.mapIdx)}</select></label><button id="gmMapMonsterStartBtn" class="btn blue" onclick="gmStartMapMonsterTest()">開始測試（${GM_TEST_RUNS} 次）</button></div><div id="gmMapMonsterTestResult" style="margin-top:12px">${result}</div></div><div class="item" style="margin-top:10px"><b>宇宙紀元 Boss</b><div class="controls" style="align-items:end;margin-top:8px"><label style="flex:1 1 320px">Boss<br><select id="gmSecondWorldBoss" class="btn" onchange="gmSecondWorldBossChange()">${secondOptions}</select></label><button id="gmSecondWorldBossStartBtn" class="btn blue" onclick="gmStartSecondWorldBossTest()">開始測試（${GM_TEST_RUNS} 次）</button></div><div class="muted" style="margin-top:7px">使用正式宇宙 Boss 能力公式＋Boss 隨機特性；不結算 EXP、暗物質、暗能量、裝備或主線進度。</div><div id="gmSecondWorldBossTestResult" style="margin-top:12px">${secondResult}</div></div>`;}
