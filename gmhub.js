(function(){
 ...
 function dungeonManagementHtml(){const d=dungeonStateSafe();return `<div class="notice gm-hub-note">目前：${Math.round((Number(d.progress)||0)*100)/100}%　／　${Math.floor(Number(d.attempts)||0)} 次　／　VIP${state.vipLevel||0}　／　${Math.floor(Number(state.vipPoints)||0)} VIP 積分</div><div class="controls" style="align-items:end"><label>副本次數累積進度 %<br><input id="gmDungeonProgress" type="number" min="0" step="0.01" value="${d.progress}" style="width:170px"></label><label>副本可挑戰次數<br><input id="gmDungeonAttempts" type="number" min="0" step="1" value="${d.attempts}" style="width:150px"></label><label>VIP 積分<br><input id="gmDungeonPoints" type="number" min="0" step="1" value="${state.vipPoints||0}" style="width:150px"></label><button class="btn blue" onclick="gmApplyDungeonValues()">套用</button></div><div class="muted" style="margin-top:7px">調低 VIP 積分不會降低已解鎖 VIP 等級；調高至新門檻時可正常升級。</div>`;}
