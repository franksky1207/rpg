(function(){
 ...
 function gearOptions(){return {quality:QUALITY.map((q,i)=>`<option value="${i}" ${i===3?"selected":""}>${q.n}</option>`).join(""),level:Array.from({length:50},(_,i)=>{const lv=i+1;return `<option value="${lv}" ${lv===state.level?"selected":""}>Lv.${lv}</option>`}).join(""),type:EQUIPMENT_TYPES.map(type=>`<option value="${type}">${equipmentTypeLabel(type)}</option>`).join("")};}
 function dungeonMapOptions(){return MAPS.map((map,i)=>`<option value="${i}">${i+1}. ${map.name}（Lv.${map.min}～${map.max}）</option>`).join("");}
 ...
 function generalManagementHtml(){
  const g=gearOptions();
  return `<div class="controls"><button class="btn" onclick="gmLevel()">指定等級</button><button class="btn" onclick="gmGold()">指定金幣</button><button class="btn" onclick="gmUnlock()">解鎖全部地圖與怪物</button><button class="btn" onclick="gmHeal()">補滿 HP</button><button class="btn danger" onclick="gmClearInventory()">清空背包</button></div>
