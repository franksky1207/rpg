(function(){
 ...
 if(!pipelineSource)fail("BOSS_CONTINUOUS_PIPELINE_API","runBattles 未載入");
  ...
  if(!/consumeMainCatchUpDelay\s*\(\s*battleGapMs\s*\(\s*r\.e\.kind\s*\)\s*\)/.test(pipelineSource))fail("BOSS_PIPELINE_FLOW_SLEEP","主線場間等待應透過 Fast Catch-up delay owner 消耗背景 credit／正常等待",pipelineSource);
 ...
 if(typeof window.backgroundProgressConsumeCatchUpCredit!=="function"||Number(window.STRUCTURED_COMBAT_HEADLESS_DURATION_VERSION)!==1||typeof window.structuredCombatPresentationDurationMs!=="function")fail("BOSS_FAST_CATCH_UP_TIMING_OWNER","Fast Catch-up 必須保留正式 Structured Combat 時間語意",{consume:typeof window.backgroundProgressConsumeCatchUpCredit,durationVersion:window.STRUCTURED_COMBAT_HEADLESS_DURATION_VERSION,duration:typeof window.structuredCombatPresentationDurationMs});
