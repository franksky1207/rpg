(function(){
 ...
 function combatAssessmentHtml(a,p){
  ...
  let value="尚未評估",detail=`以 ${target} 目前的「${pos}」位置算法進行 500 次完整三連戰。`;
  ...
  return `<div class="arena-dual-card ${assessmentTone(a)}"><div class="arena-dual-head"><span>① 戰力評估</span><strong>${value}</strong></div><div class="arena-dual-target">目前最高：${target}・${pos}位置</div><div class="arena-dual-bar"><span style="width:${pct}%"></span></div><p>${detail}</p></div>`;
 }
 ...
 function venueCard(rank,p,d){
  ...
  const pointText=cfg?`三戰全通 ${cfg.totalPoints} VIP 積分・${cfg.stagePoints.join(" + ")}`:"三戰挑戰";
 ...
 function venueSelectionHtml(){
  ...
  return `<div class="function-page dungeon-page-shell arena-shell arena-venue-page"><div class="back-home"><button class="btn back-btn" onclick="go('dungeon')">← 返回副本</button></div><section class="arena-panel"><div class="arena-title">競技場</div><div class="arena-attempts">目前可挑戰次數：<strong>${d.attempts}</strong> 次・${visibleText}</div><div class="muted" style="margin:8px 0 12px">競技場名稱依主線區域；目前畫面由左至右依序套用低／中／高位置算法。HP／ATK／DEF 則只看競技場實際階層。</div>${unlockPanelHtml(a,p)}<div class="arena-venue-grid">${cards}</div></section></div>`;
 }
