...
function check(value,where){if(hasEnglish(value))failures.push({where,text:String(value)});}
...
if(failures.length){
 ...
 failures.forEach((row,index)=>console.error(`${index+1}. ${row.where}: ${row.text}`));
 process.exit(1);
}
